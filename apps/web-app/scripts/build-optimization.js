...
if (fs.existsSync(staticDir)) {
  ...
  console.log(
    `✅ Asset manifest generated with ${Object.keys(manifest.assets).length} assets and ${Object.keys(manifest.chunks).length} chunks`,
  );
  optimizations.push(
    `Asset manifest with ${Object.keys(manifest.assets).length} entries`,
  );
}
...
if (fs.existsSync(assetsDir)) {
  ...
  console.log(
    `   - Total size: ${Math.round((assetStats.totalSize / 1024 / 1024) * 100) / 100}MB`,
  );
