...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const response = await fetch(`${railsApiUrl}/api/v1/mentoring/requests?${queryParams}`, {
        method: 'GET',
        headers: {
      ...
      return NextResponse.json({
        requests: requestsData.requests?.map((request: any) => ({
          uuid: request.uuid,
          student: {
          ...
          links: {
            ...
            discussion: request.discussion_uuid ? `/mentoring/discussions/${request.discussion_uuid}` : null
          }
        })) || [],
        meta: {
