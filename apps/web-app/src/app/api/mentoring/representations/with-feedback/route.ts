 ...
 * GET /api/mentoring/representations/with-feedback - List representations with feedback
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Connect to Rails API to fetch representations with feedback
    ...
    try {
      ...
      const response = await fetch(`${railsApiUrl}/api/v1/mentoring/representations/with_feedback?${queryParams}`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        return NextResponse.json(
          { error: 'Failed to fetch representations with feedback' },
      ...
      return NextResponse.json({
        results: representationsData.representations?.map((representation: RailsMentoringRepresentationResponse) => ({
          ...
          links: {
            self: `/mentoring/automation/representations/${representation.id}?from=with-feedback`
          }
        })) || [],
        meta: {
    ...
    console.error('Representations with feedback fetch error:', error)
    return NextResponse.json(
