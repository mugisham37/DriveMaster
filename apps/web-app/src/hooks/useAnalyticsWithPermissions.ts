...
export function useAnalyticsWithPermissions() {
  ...
  const userContext = useMemo(() => {
    ...
    const role: UserRole = user.isMentor
      ? "mentor"
      : user.isInsider
        ? "admin"
        : "learner";
 ...
 */
export function useEngagementMetricsWithPermissions(
  params?: EngagementMetricsParams,
) {
  const { client, userContext, isServiceAvailable, isAuthenticated } =
  ...
  const shouldFetch = Boolean(
    isServiceAvailable &&
      ...
      userContext &&
      filteredRequest &&
      filteredRequest.restrictions.length === 0,
  );
 ...
 */
export function useHourlyEngagementWithPermissions(
  userId: string,
  date?: Date,
) {
  const { client, userContext, isServiceAvailable, isAuthenticated } =
  ...
  const shouldFetch = Boolean(
    isServiceAvailable && isAuthenticated && canAccess && !!userId,
  );
 ...
 */
export function useProgressMetricsWithPermissions(
 ...
 */
export function useContentMetricsWithPermissions(
...
export function useContentGapsWithPermissions() {
  ...
  const shouldFetch = Boolean(
    isServiceAvailable && isAuthenticated && canAccess,
  );
...
export function useAlertsWithPermissions(severity?: AlertSeverity) {
  ...
  return useQuery({
    ...
    select: (data: Alert[]) => ({
      ...
      _metadata: {
        canManage: userContext
          ? shouldRenderComponent("alert-management", userContext)
          : false,
        dataViewLevel: userContext ? getDataViewLevel(userContext) : "summary",
      },
    }),
...
export function useEffectivenessReportWithPermissions(filters?: ReportFilters) {
  ...
  return useQuery({
    queryKey: analyticsQueryKeys.effectivenessReport(
      filteredRequest?.filteredParams,
    ),
    queryFn: () => {
      if (!filteredRequest) {
        throw new Error("No filtered request available");
...
export function useHistoricalMetricsWithPermissions(query: HistoricalQuery) {
  ...
  return useQuery({
    queryKey: analyticsQueryKeys.historical(
      filteredRequest?.filteredParams || query,
    ),
    queryFn: () => {
      if (!filteredRequest) {
        throw new Error("No filtered request available");
...
export function useAnalyticsComponentVisibility() {
  ...
  return useMemo(() => {
    ...
    return {
      showEngagement: shouldRenderComponent(
