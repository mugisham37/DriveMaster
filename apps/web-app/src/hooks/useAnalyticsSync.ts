...
export function useAnalyticsCrossTabSync(options: AnalyticsSyncOptions = {}) {
  ...
  const broadcastFilterChange = useCallback(
  ...
  const broadcastTimeRangeChange = useCallback(
  ...
  const broadcastCacheInvalidation = useCallback(
    (queryKey?: unknown[]) => {
 ...
 */
export function useAnalyticsOptimisticUpdates(
...
export function useAnalyticsMutationsWithOptimistic() {
  ...
  const updateEngagementMutation = useMutation({
    mutationFn: async (data: {
      userId: string;
      engagement: Partial<UserEngagementMetrics>;
    }) => {
