...
export const notificationQueryKeys = {
  ...
  userCounts: (userId: string) =>
    [...notificationQueryKeys.counts(), userId] as const,
};
...
export function useNotifications(
  ...
  useEffect(() => {
        ...
        (oldData) => {
          ...
          return {
            ...
            meta: {
              ...
              unreadCount: notification.status.isRead
                ? oldData.meta.unreadCount
                : oldData.meta.unreadCount + 1,
            },
          };
        },
      );
    ...
    const handleNotificationUpdated = (notification: Notification) => {
      ...
      queryClient.setQueryData<NotificationList>(
        ...
        (oldData) => {
          ...
          const unreadCount = updatedResults.filter(
            (n) => !n.status.isRead,
          ).length;
    ...
    const handleNotificationDeleted = (notificationId: string) => {
      ...
      queryClient.setQueryData<NotificationList>(
        ...
        (oldData) => {
          ...
          const deletedNotification = oldData.results.find(
            (n) => n.id === notificationId,
          );
          const updatedResults = oldData.results.filter(
            (n) => n.id !== notificationId,
          );
          ...
          return {
            ...
            meta: {
              ...
              unreadCount:
                deletedNotification && !deletedNotification.status.isRead
    ...
    if (params.priority) {
      filters.priorities = Array.isArray(params.priority)
        ? params.priority
        : [params.priority];
    }
    ...
    const subscriptionId = wsClient.subscribeToUserNotifications(
      user.id.toString(),
      filters,
    );
...
export function useNotificationMutations(): UseNotificationMutationsResult {
  ...
  const markAsReadMutation = useMutation({
    ...
    onSettled: () => {
      ...
      if (user?.id) {
        queryClient.invalidateQueries({
          queryKey: notificationQueryKeys.userCounts(
...
export function useNotificationCounts(): UseNotificationCountsResult {
  ...
  const query = useQuery({
    ...
    queryFn: () => {
      if (!user?.id) throw new Error("User not authenticated");
      return notificationApiClient.getNotificationCounts(
        requireStringUserId(user.id),
      );
    },
    enabled: !!user?.id,
