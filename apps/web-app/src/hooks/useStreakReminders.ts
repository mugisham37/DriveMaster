...
export interface UseStreakRemindersResult {
  ...
  updateStreakReminder: (
    reminderId: string,
    updates: Partial<StreakReminderRequest>,
  ) => Promise<void>;
  ...
  getStreakStats: (
    streakType?: "daily" | "weekly" | "monthly",
  ) => Promise<StreakData | Record<string, StreakData>>;
  setStreakGoal: (
    streakType: "daily" | "weekly" | "monthly",
    goal: number,
  ) => Promise<void>;
...
export function useStreakReminders(
  ...
  const [streakData, setStreakData] = useState<
    Record<"daily" | "weekly" | "monthly", StreakData | null>
  >({
    daily: null,
    weekly: null,
    monthly: null,
      ...
      return response.notifications.map(
        (notification) =>
          ({
            ...
            streakCount:
              Number(notification.notification.data?.streakCount) || 0,
            streakType: (notification.notification.data?.streakType ||
              "daily") as "daily" | "weekly" | "monthly",
            reminderTime: notification.scheduledFor,
            motivationalMessage: notification.notification.data
              ?.motivationalMessage as string | undefined,
            streakGoal:
              Number(notification.notification.data?.streakGoal) || undefined,
            ...
            streakData: {
              ...
              currentStreak:
                Number(notification.notification.data?.streakCount) || 0,
              longestStreak:
                Number(notification.notification.data?.longestStreak) || 0,
              lastActivityDate: notification.notification.data?.lastActivityDate
                ? new Date(
                    String(notification.notification.data.lastActivityDate),
                  )
                : new Date(),
              streakGoal:
                Number(notification.notification.data?.streakGoal) || undefined,
              isActive: true,
              streakStartDate: notification.notification.data?.streakStartDate
                ? new Date(
                    String(notification.notification.data.streakStartDate),
                  )
                : new Date(),
              totalActiveDays:
                Number(notification.notification.data?.totalActiveDays) || 0,
            },
            isStreakBroken: Boolean(
              notification.notification.data?.isStreakBroken,
            ),
            daysSinceLastActivity:
              Number(notification.notification.data?.daysSinceLastActivity) ||
              0,
            riskLevel: (notification.notification.data?.riskLevel || "safe") as
              | "safe"
              | "at-risk"
              | "critical"
              | "broken",
  ...
  const cancelReminderMutation = useMutation({
    mutationFn: async (reminderId: string) => {
      await notificationApiClient.cancelScheduledNotification(
        reminderId,
        "User cancelled",
      );
    },
  ...
  const snoozeReminderMutation = useMutation({
    mutationFn: async ({
      ...
      await notificationApiClient.rescheduleNotification(
        reminderId,
        newScheduledTime,
      );
    },
  ...
  const recordActivity = useCallback(
    async (streakType: "daily" | "weekly" | "monthly") => {
      if (!user?.id) return;
      ...
      const updatedStreakData: StreakData = {
        ...
        streakGoal:
          currentData?.streakGoal || config.defaultStreakGoals[streakType],
        isActive: true,
        streakStartDate: isNewStreak
          ? now
          : currentData?.streakStartDate || now,
      ...
      await notificationApiClient.trackClick(
        `streak-${streakType}`,
        String(user.id),
        `activity_recorded_${newStreakCount}`,
      );
    },
  ...
  const setStreakGoal = useCallback(
    async (streakType: "daily" | "weekly" | "monthly", goal: number) => {
      if (!user?.id) return;
  ...
  const getStreakAnalytics = useCallback(
    async (streakType?: "daily" | "weekly" | "monthly") => {
      ...
      const allStreaks = Object.values(streakData).filter(
        Boolean,
      ) as StreakData[];
      return {
        totalActiveDays: allStreaks.reduce(
          (sum, streak) => sum + streak.totalActiveDays,
          0,
        ),
        longestStreak: Math.max(
          ...allStreaks.map((streak) => streak.longestStreak),
        ),
        currentStreak: Math.max(
          ...allStreaks.map((streak) => streak.currentStreak),
        ),
        streakBreaks: 0,
