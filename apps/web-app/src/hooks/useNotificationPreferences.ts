...
export function useNotificationPreferences(): UseNotificationPreferencesResult {
  ...
  const updatePreferencesMutation = useMutation({
    mutationFn: (updates: Partial<NotificationPreferences>) => {
      if (!user?.id) throw new Error("User not authenticated");
      return notificationApiClient.updatePreferences(
        user.id.toString(),
        updates,
      );
    },
  ...
  const updateEnabledTypes = useCallback(
    (types: NotificationType[]) =>
      updatePreferencesMutation.mutateAsync({ enabledTypes: types }),
    [updatePreferencesMutation],
  );

  const updateQuietHours = useCallback(
    (quietHours: QuietHours) =>
      updatePreferencesMutation.mutateAsync({ quietHours }),
    [updatePreferencesMutation],
  );
  ...
  const updateGlobalSettings = useCallback(
    (settings: GlobalNotificationSettings) =>
      updatePreferencesMutation.mutateAsync({ globalSettings: settings }),
    [updatePreferencesMutation],
  );
  ...
  useEffect(() => {
    ...
    const subscriptionId = wsClient.subscribeToPreferenceChanges(
      user.id.toString(),
    );
...
export function usePreferenceValidation() {
  const validateQuietHours = useCallback(
    (quietHours: QuietHours): PreferenceValidationResult => {
      const errors: string[] = [];
      ...
      if (quietHours.enabled) {
        ...
        if (errors.length === 0) {
          ...
          if (duration < 60) {
            // Less than 1 hour
            warnings.push(
              "Quiet hours duration is very short (less than 1 hour)",
            );
          }
        }
        ...
        if (quietHours.daysOfWeek) {
          const invalidDays = quietHours.daysOfWeek.filter(
            (day) => day < 0 || day > 6,
          );
  ...
  const validateFrequencySettings = useCallback(
    (settings: FrequencySettings): PreferenceValidationResult => {
      const errors: string[] = [];
      const warnings: string[] = [];
      ...
      switch (settings.type) {
        ...
        case "daily":
          if (!settings.dailyTime) {
            ...
            !/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(settings.dailyTime)
          ) {
        ...
        case "weekly":
          if (
            settings.weeklyDay === undefined ||
            settings.weeklyDay < 0 ||
            settings.weeklyDay > 6
          ) {
            errors.push(
              "Weekly day must be between 0 (Sunday) and 6 (Saturday)",
            );
          }
          if (!settings.weeklyTime) {
            errors.push("Weekly time is required for weekly frequency");
          } else if (
            !/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(settings.weeklyTime)
          ) {
  ...
  const validateGlobalSettings = useCallback(
    (settings: GlobalNotificationSettings): PreferenceValidationResult => {
      const errors: string[] = [];
      const warnings: string[] = [];
      ...
      if (settings.maxPerDay !== undefined) {
        ...
        if (settings.maxPerDay < 10) {
          warnings.push(
            "Very low daily notification limit may cause important notifications to be missed",
          );
        }
      }
