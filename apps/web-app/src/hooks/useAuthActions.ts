...
import { useStableAuthCallback } from "@/lib/auth/performance-optimization";
import type {
  LoginCredentials,
  RegisterData,
  OAuthProviderType,
...
function validateRegistrationData(userData: RegisterData): ValidationResult {
  ...
  if (
    userData.timezone &&
    !Intl.supportedValuesOf("timeZone").includes(userData.timezone)
  ) {
...
export function useAuthActions(): UseAuthActionsReturn {
  ...
  const validateAndLogin = useStableAuthCallback(
    async (credentials: LoginCredentials, options: LoginOptions = {}) => {
  ...
  const register = useStableAuthCallback(
    async (userData: RegisterData, options: RegisterOptions = {}) => {
      try {
  ...
  const validateAndRegister = useStableAuthCallback(
    async (userData: RegisterData, options: RegisterOptions = {}) => {
      // Skip validation if explicitly requested
      if (!options.skipValidation) {
  ...
  const logout = useStableAuthCallback(
  ...
  const isRecoverableError = useStableAuthCallback(
    (error: AuthError): boolean => {
  ...
  const initiateOAuth = useStableAuthCallback(
    async (provider: OAuthProviderType, options: OAuthOptions = {}) => {
      try {
  ...
  const handleOAuthCallback = useStableAuthCallback(
    async (provider: OAuthProviderType, code: string, state: string) => {
      try {
        ...
        if (response && typeof response === "object" && "user" in response) {
          ...
          const redirectTo =
            sessionStorage.getItem("oauth_redirect_url") || "/dashboard";
  ...
  const linkOAuthProvider = useStableAuthCallback(
    async (provider: OAuthProviderType, code: string, state: string) => {
      try {
  ...
  const unlinkOAuthProvider = useStableAuthCallback(
  ...
  const validateCredentials = useStableAuthCallback(
  ...
  const validateRegistrationDataCallback = useStableAuthCallback(
    (userData: RegisterData): ValidationResult => {
