        ...
        pollerRef.current = createJobPoller(
          (jobId) => contentServiceClient.getBulkOperationStatus(jobId),
          {
            ...
            onStatusChange: (operation, previousStatus) => {
              console.log(`Import status changed: ${previousStatus} -> ${operation.status}`)
            },
            onError: (error) => {
...
export function useBulkExport() {
      ...
      if (result.status !== 'completed' && result.status !== 'failed') {
        ...
        pollerRef.current = createJobPoller(
          (jobId) => contentServiceClient.getBulkOperationStatus(jobId),
          {
            ...
            onStatusChange: (operation, previousStatus) => {
              console.log(`Export status changed: ${previousStatus} -> ${operation.status}`)
  ...
  const downloadFile = useCallback(async () => {
    ...
    try {
      const { blob, filename } = await contentServiceClient.downloadExportResult(state.result.operationId)
...
export function useCsvPreview() {
  ...
  const previewCsv = useCallback(async (csvData: string, options?: { limit?: number }) => {
    try {
      setState(prev => ({ ...prev, isLoading: true, error: null }))

      const result = await contentServiceClient.previewCsvImport(csvData, options)
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error : new Error('CSV preview failed'),
        isLoading: false
      }))
    }
  }, [])
...
export function useBulkOperationsList() {
  ...
  const loadOperations = useCallback(async (options?: {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        error: error instanceof Error ? error : new Error('Failed to load operations'),
        isLoading: false
      }))
    }
  }, [])
