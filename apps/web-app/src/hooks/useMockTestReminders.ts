...
export interface UseMockTestRemindersResult {
  ...
  updateReminder: (
    reminderId: string,
    updates: Partial<MockTestReminderRequest>,
  ) => Promise<void>;
...
export function useMockTestReminders(
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return response.notifications.map((notification) => {
        const data =
          (notification.notification.data as Record<string, unknown>) || {};
        ...
        if (data.difficultyLevel) {
          reminder.difficultyLevel = data.difficultyLevel as
            | "beginner"
            | "intermediate"
            | "advanced";
        }
        ...
        if (data.nextSuggestedTest) {
          reminder.nextSuggestedTest = new Date(
            data.nextSuggestedTest as string | number | Date,
          );
        }
  ...
  const recordTestResult = useCallback(
    async (result: Omit<TestResult, "id">) => {
      ...
      await notificationApiClient.trackClick(
        ...
        String(user.id),
        `completed_score_${result.score}`,
      );

      // Update reminder data if applicable
      const relatedReminder = scheduledReminders.find(
        (r) => r.testType === result.testType && r.testName === result.testName,
      );

      if (relatedReminder) {
        const updatedHistory = [
          newResult,
          ...(relatedReminder.testHistory || []),
        ];
        const averageScore =
          updatedHistory.reduce((sum, test) => sum + test.score, 0) /
  ...
  const getTestAnalytics = useCallback(
    async (testType?: string) => {
      ...
      const averageScore =
        relevantTests.reduce((sum, test) => sum + test.score, 0) /
        relevantTests.length;
      const passRate =
        (relevantTests.filter((test) => test.score >= 70).length /
          relevantTests.length) *
        100;
      ...
      const topicAverages = Object.entries(topicScores).map(
        ([topic, scores]) => ({
          topic,
          average:
  ...
  const getSuggestedTestTime = useCallback(
    async (
      testType: string,
      difficultyLevel: "beginner" | "intermediate" | "advanced",
    ): Promise<Date> => {
      ...
      const userTests = testHistory.filter(
      ...
      if (recentPerformance.length > 0) {
        ...
        const hourAverages = Object.entries(hourPerformance).map(
          ([hour, scores]) => ({
            hour: parseInt(hour),
            average:
              scores.reduce((sum, score) => sum + score, 0) / scores.length,
        ...
        if (hourAverages.length > 0) {
          const sortedAverages = hourAverages.sort(
  ...
  const getPersonalizedPreparationTips = useCallback(
    (_testType: string, userPerformance: number): string[] => {
      const difficultyTips =
        userPerformance >= 80
          ? PREPARATION_TIPS.advanced
          : userPerformance >= 60
            ? PREPARATION_TIPS.intermediate
            : PREPARATION_TIPS.beginner;

      const performanceTips =
        userPerformance >= 80
          ? PERFORMANCE_BASED_TIPS.high
          : userPerformance >= 60
            ? PERFORMANCE_BASED_TIPS.medium
  ...
  const getPerformanceTrends = useCallback(
    async (timeframe: "week" | "month" | "quarter") => {
      const now = new Date();
      const timeframeDays =
      ...
      const relevantTests = testHistory.filter(
        (test) => test.completedAt >= cutoffDate,
      );
  ...
  const calculateImprovementTrend = (tests: TestResult[]): number => {
    ...
    const recentAvg =
      ...
      recentTests.length;
    const olderAvg =
      olderTests.reduce((sum, test) => sum + test.score, 0) / olderTests.length;
