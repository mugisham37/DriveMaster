...
export function useNotificationPerformance(
  ...
  const [performanceManager, setPerformanceManager] =
    useState<NotificationPerformanceManager | null>(null);
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      const manager = createNotificationPerformanceManager(
      ...
      console.log(
        "[useNotificationPerformance] Performance manager initialized",
      );
    } catch (err) {
      const error =
        err instanceof Error
          ? err
          : new Error("Failed to initialize performance manager");
      setError(error);
      console.error(
        "[useNotificationPerformance] Initialization failed:",
        error,
      );
    } finally {
      setIsLoading(false);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const error =
        err instanceof Error
          ? err
          : new Error("Failed to refresh performance data");
      setError(error);
   ...
   */
  const recordUIOperation = useCallback(
  ...
  const updateCacheMetrics = useCallback(
    (hits: number, misses: number, size: number, evictions: number = 0) => {
      if (performanceManager) {
   ...
   */
  const executeOptimizedRequest = useCallback(
    async <T>(
      operation: string,
      params: unknown,
      ...
      return performanceManager.executeOptimizedRequest<T>(
        operation,
        params,
        priority,
      );
...
export function useUIPerformanceMeasurement() {
  const { recordUIOperation } = useNotificationPerformance({
