...
export function usePaginatedRequestQuery<TData, TError = Error>(
  ...
  const queryOptions: UseQueryOptions<TData, TError> = {
    ...
    ...(request.options?.enabled !== undefined && {
      enabled: request.options.enabled,
    }),
    ...(request.options?.staleTime !== undefined && {
      staleTime: request.options.staleTime,
    }),
    ...(request.options?.gcTime !== undefined && {
      gcTime: request.options.gcTime,
    }),
    ...(request.options?.initialData !== undefined && {
