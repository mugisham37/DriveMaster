  ...
  useEffect(() => {
    ...
    const initializeManager = async () => {
      try {
        ...
        console.error(
          "[usePerformanceManager] Failed to initialize performance manager:",
          error,
        );
      }
    };
...
export function usePerformanceOptimization() {
  const { optimizeRequest, executePrefetch, recordNavigation } =
...
export function usePerformanceMonitoring() {
  const { stats, getHealthSummary, logPerformanceSummary } =
    usePerformanceManager();
