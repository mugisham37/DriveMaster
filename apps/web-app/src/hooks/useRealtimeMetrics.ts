 ...
 */
export function useRealtimeMetrics(options: RealtimeMetricsOptions = {}): RealtimeMetricsResult {
  const {
    enabled = true,
    autoConnect = true,
  ...
  const handleMetricsUpdate = useCallback((message: MetricsUpdate) => {
  ...
  const handleAlertsUpdate = useCallback((message: AlertMessage) => {
    ...
    const hasCriticalAlerts = message.alerts.some(alert => alert.severity === 'critical')
    if (hasCriticalAlerts) {
      queryClient.invalidateQueries({ queryKey: analyticsQueryKeys.system() })
    }
  }, [queryClient])
  ...
  }, [enabled, autoConnect, subscriptions, connect, disconnect, subscribe, handleConnectionChange, handleError])
...
function updateEngagementCache(
  ...
  if (strategy.merge) {
    ...
    queryClient.setQueryData(queryKey, (oldData: UserEngagementMetrics | undefined) => {
  ...
  if (strategy.invalidateRelated) {
    ...
    queryClient.invalidateQueries({ 
      predicate: (query) => query.queryKey[0] === 'analytics' && query.queryKey[2] === 'hourly'
    })
  }
}
...
function updateProgressCache(
  ...
  if (strategy.merge) {
    queryClient.setQueryData(queryKey, (oldData: LearningProgressMetrics | undefined) => {
      if (!oldData) return data
  ...
  if (strategy.invalidateRelated) {
    queryClient.invalidateQueries({ 
      predicate: (query) => query.queryKey[0] === 'analytics' && query.queryKey[2] === 'journey'
    })
    queryClient.invalidateQueries({ 
      predicate: (query) => query.queryKey[0] === 'analytics' && query.queryKey[2] === 'cohort'
    })
  }
}
...
function updateContentCache(
  ...
  if (strategy.merge) {
    queryClient.setQueryData(queryKey, (oldData: ContentPerformanceMetrics | undefined) => {
      if (!oldData) return data
  ...
  if (strategy.invalidateRelated) {
    queryClient.invalidateQueries({ queryKey: analyticsQueryKeys.contentGaps() })
  }
}
...
function updateSystemCache(
  ...
  if (strategy.merge) {
    queryClient.setQueryData(queryKey, (oldData: SystemPerformanceMetrics | undefined) => {
      if (!oldData) return data
  ...
  if (strategy.invalidateRelated) {
    queryClient.invalidateQueries({ queryKey: analyticsQueryKeys.systemStatus() })
