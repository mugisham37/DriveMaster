...
export function useSessionTimeout(
  ...
  const updateActivity = useCallback(() => {
    ...
    const timeSinceLastUpdate =
      now.getTime() - lastActivityUpdateRef.current.getTime();
  ...
  const checkSessionStatus = useCallback(() => {
    ...
    if (
      timeRemaining <= configRef.current.warningTime &&
      ...
      console.log(
        `Session warning: ${Math.ceil(timeRemaining / 1000)} seconds remaining`,
      );
    }
  ...
  const isSessionValid =
    state.isActive && !state.isExpired && state.timeRemaining > 0;
  const shouldShowWarning = state.isWarningShown && !state.isExpired;
  const timeUntilWarning = Math.max(
    0,
    state.timeRemaining - configRef.current.warningTime,
  );
  ...
  const updateConfig = useCallback(
    (newConfig: Partial<SessionTimeoutConfig>) => {
...
export function useSessionWarning(): UseSessionWarningReturn {
  ...
  const sessionTimeout = useSessionTimeout({
    onWarning: (timeRemaining) => {
      console.log(
        `Session warning triggered: ${Math.ceil(timeRemaining / 1000)}s remaining`,
      );
    },
    onTimeout: () => {
  ...
  return {
    ...
    timeRemainingFormatted: formatTimeRemaining(
      sessionTimeout.timeUntilExpiration,
    ),
