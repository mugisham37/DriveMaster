...
export interface SpacedRepetitionReminderDisplay
  extends SpacedRepetitionRequest {
  id: string;
  scheduledFor: Date;
  isActive: boolean;
  reviewCount: number;
...
export interface UseSpacedRepetitionRemindersResult {
  ...
  updateReminder: (
    reminderId: string,
    updates: Partial<SpacedRepetitionRequest>,
  ) => Promise<void>;
...
export function useSpacedRepetitionReminders(
      ...
      return response.notifications.map(
        (notification) =>
          ({
            ...
            difficulty: (notification.notification.data?.difficulty ||
              config.defaultDifficulty) as "easy" | "medium" | "hard",
            lastReviewDate: notification.notification.data?.lastReviewDate
              ? new Date(String(notification.notification.data.lastReviewDate))
              : undefined,
            nextReviewDate: notification.scheduledFor,
            scheduledFor: notification.scheduledFor,
            isActive: notification.status === "pending",
            reviewCount:
              Number(notification.notification.data?.reviewCount) || 0,
            averageScore: notification.notification.data?.averageScore as
              | number
              | undefined,
            retentionRate: notification.notification.data?.retentionRate as
              | number
              | undefined,
  ...
  const scheduleReminderMutation = useMutation({
    mutationFn: async (request: SpacedRepetitionRequest) => {
      ...
      return await notificationApiClient.scheduleSpacedRepetitionReminder(
        enhancedRequest,
      );
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["spaced-repetition-reminders"],
      });
      if (!user) return;
      showToast({
        id: `toast-${Date.now()}`,
  ...
  const getPerformanceLevel = (
    score: number,
  ): keyof typeof PERFORMANCE_MULTIPLIERS => {
  ...
  const calculateNextReviewDate = useCallback(
    (reminder: SpacedRepetitionReminderDisplay, score: number): Date => {
      ...
      const intervalIndex = Math.min(
        reviewCount,
        SPACED_REPETITION_INTERVALS[reminder.difficulty].length - 1,
      );
      const baseInterval =
        SPACED_REPETITION_INTERVALS[reminder.difficulty][intervalIndex] ?? 1;
  ...
  const adjustDifficultyBasedOnScore = (
  ...
  const recordReviewSession = useCallback(
    async (reminderId: string, score: number, timeSpent: number) => {
      if (!user?.id) return;
  ...
  const getOptimalSchedule = useCallback(
    async (
      ...
      const averageScore =
        performanceHistory.scores.reduce((sum, score) => sum + score, 0) /
        performanceHistory.scores.length;
      ...
      const intervalIndex = Math.min(
        reviewCount,
        SPACED_REPETITION_INTERVALS[difficulty].length - 1,
      );
      const baseInterval =
        SPACED_REPETITION_INTERVALS[difficulty][intervalIndex] ?? 1;
  ...
  const adjustDifficulty = useCallback(
    async (reminderId: string, newDifficulty: "easy" | "medium" | "hard") => {
      await updateReminderMutation.mutateAsync({
        reminderId,
  ...
  const getPerformanceStats = useCallback(
    async (topicName?: string) => {
      ...
      const retentionRate =
        allScores.length > 0 ? (passedReviews / allScores.length) * 100 : 0;
