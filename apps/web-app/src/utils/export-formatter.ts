...
export class ExportFormatter {
  ...
  private formatValue(
    ...
    if (
      ...
      (typeof value === "string" && field.includes("At"))
    ) {
  ...
  static validateOptions(options: ExportOptions): {
    ...
    if (
      options.dateFormat &&
      !["iso", "locale", "timestamp"].includes(options.dateFormat)
    ) {
      result.valid = false;
      result.errors.push(
        "Invalid date format. Supported formats: iso, locale, timestamp",
      );
    }
    ...
    if (
      options.compression &&
      !["none", "zip", "gzip"].includes(options.compression)
    ) {
      result.valid = false;
      result.errors.push(
        "Invalid compression format. Supported formats: none, zip, gzip",
      );
    }
 ...
 */
export function createExportFormatter(
  format: ExportFormat,
...
export function estimateExportSize(
  ...
  if (estimatedBytes < 1024) {
    ...
    if (size > 100) {
      warning =
        "Large export file. Consider filtering or splitting the export.";
    }
  } else {
    size = Math.round(estimatedBytes / (1024 * 1024 * 1024));
    unit = "GB";
    warning =
      "Very large export file. Consider filtering or splitting the export.";
  }
