...
export interface PollingOptions {
  ...
  onStatusChange?: (operation: BulkOperation, previousStatus?: BulkOperationStatus) => void
...
export type PollingState = 'idle' | 'polling' | 'completed' | 'error' | 'cancelled'
...
export class JobPoller {
  ...
  async poll(jobId: string): Promise<PollingResult> {
    if (this.state === 'polling') {
      throw new Error('Poller is already running. Stop current polling before starting a new one.')
    }
  ...
  private async pollUntilComplete(): Promise<BulkOperation> {
    ...
    try {
      while (this.attempts < this.options.maxAttempts && this.state === 'polling') {
  ...
  private isOperationComplete(operation: BulkOperation): boolean {
    const completedStatuses: BulkOperationStatus[] = ['completed', 'failed', 'cancelled']
  ...
  private calculateInterval(): number {
    ...
    const backoffInterval = baseInterval * Math.pow(this.options.backoffFactor, this.attempts - 1)
...
export class ProgressTracker {
  ...
  updateProgress(operation: BulkOperation): void {
    const progress: ProgressUpdate = {
      ...
      percentage: operation.totalItems > 0 ? Math.round((operation.processedItems / operation.totalItems) * 100) : 0,
   ...
   */
  private calculateEstimatedTimeRemaining(operation: BulkOperation): number | null {
...
export interface ProgressUpdate {
  ...
  errors: Array<{ itemId?: string; error: string; details?: string; timestamp: Date }>
  timestamp: Date
}
...
export function formatProgress(progress: ProgressUpdate): {
  ...
  const errorSummary = progress.failedItems > 0 
