...
export class CircuitBreaker {
  ...
  private listeners: Array<
    (state: CircuitBreakerState, stats: CircuitBreakerStats) => void
  > = [];
  ...
  private transitionToOpen(): void {
    ...
    this.nextRetryTime = new Date(
      Date.now() + (this.config.recoveryTimeout ?? 30000),
    );
    this.recordStateChange();

    console.log(
      `[CircuitBreaker] Transitioned to OPEN state. Next retry at: ${this.nextRetryTime.toISOString()}`,
    );
  }
  ...
  private getRetryAfterSeconds(): number {
    ...
    return Math.max(
      0,
      Math.ceil((this.nextRetryTime.getTime() - Date.now()) / 1000),
    );
  }
  ...
  getMetrics(): CircuitBreakerMetrics {
    ...
    const requestsPerSecond =
      monitoringPeriodMs > 0
        ? (stats.totalRequests / monitoringPeriodMs) * 1000
        : 0;
   ...
   */
  addListener(
    listener: (state: CircuitBreakerState, stats: CircuitBreakerStats) => void,
  ): () => void {
  ...
  private notifyListeners(): void {
    ...
    this.listeners.forEach((listener) => {
      try {
        ...
        console.error(
          "[CircuitBreaker] Error in state change listener:",
          error,
        );
      }
    });
...
export function isCircuitBreakerError(error: unknown): boolean {
  ...
  if (error && typeof error === "object" && "type" in error) {
    ...
    return ["network", "timeout", "server", "service_unavailable"].includes(
      errorType,
    );
  }
