...
export class GracefulDegradationManager {
  ...
  async executeWithDegradation<T>(
    ...
    try {
      ...
      return this.applyFallbackStrategy(fallbackKey, fallbackStrategy, error as ContentServiceError)
    }
  }
  ...
  saveToLocalStorage<T>(key: string, data: CachedData<T>): void {
    try {
      ...
      console.warn('[GracefulDegradation] Failed to save to localStorage:', error)
    }
  }
  ...
  private generatePlaceholder<T>(key: string): T {
    if (key.includes('content')) {
      return {
        ...
        content: { body: 'This content is temporarily unavailable. Please try again later.' },
        type: 'placeholder',
        status: 'draft'
      } as T
    }
  ...
  queueOfflineAction(
    ...
    console.log(`[GracefulDegradation] Queued offline action: ${type} ${resource}`)
  }
  ...
  async processOfflineQueue(): Promise<void> {
    if (this.serviceHealth.status !== 'online' || this.offlineQueue.length === 0) {
      return
    }

    console.log(`[GracefulDegradation] Processing ${this.offlineQueue.length} queued actions`)
    ...
    for (const action of actionsToProcess) {
      try {
        ...
        console.log(`[GracefulDegradation] Successfully processed action: ${action.id}`)
      } catch (error) {
        action.retryCount++
        ...
        if (action.retryCount < action.maxRetries) {
          // Re-queue for retry
          this.offlineQueue.push(action)
          console.warn(`[GracefulDegradation] Action failed, will retry: ${action.id}`)
        } else {
          console.error(`[GracefulDegradation] Action failed permanently: ${action.id}`, error)
        }
      }
    }
  ...
  private updateServiceHealth(success: boolean): void {
    ...
    if (success) {
      if (this.serviceHealth.errorRate > 0) {
        this.serviceHealth.errorRate = Math.max(0, this.serviceHealth.errorRate - 5)
      }
    } else {
      this.serviceHealth.errorRate = Math.min(100, this.serviceHealth.errorRate + 10)
    }
    ...
    } else if (this.serviceHealth.errorRate >= this.config.degradationThreshold) {
    ...
    if (previousStatus !== this.serviceHealth.status) {
      console.log(`[GracefulDegradation] Service status changed: ${previousStatus} -> ${this.serviceHealth.status}`)
  ...
  private loadOfflineQueue(): void {
    try {
      ...
      if (stored) {
        ...
        console.log(`[GracefulDegradation] Loaded ${this.offlineQueue.length} queued actions`)
      }
    } catch (error) {
      console.warn('[GracefulDegradation] Failed to load offline queue:', error)
      this.offlineQueue = []
    }
  }
  ...
  private saveOfflineQueue(): void {
    try {
      ...
      localStorage.setItem('content_service_offline_queue', JSON.stringify(toStore))
    } catch (error) {
      console.warn('[GracefulDegradation] Failed to save offline queue:', error)
    }
  }
  ...
  getOfflineQueueStatus(): {
    ...
    const oldestAction = new Date(Math.min(...timestamps.map(t => t.getTime())))
    const newestAction = new Date(Math.max(...timestamps.map(t => t.getTime())))
 ...
 */
export function createGracefulDegradationManager(config?: Partial<DegradationConfig>): GracefulDegradationManager {
...
export function getNetworkStatus(): {
  ...
  const result: { online: boolean; effectiveType?: string; downlink?: number; rtt?: number } = {
    online: navigator.onLine
  }

  if (connection?.effectiveType) result.effectiveType = connection.effectiveType
