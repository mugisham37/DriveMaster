...
export class PerformanceMonitor {
   ...
   */
  recordRequest(operation: string, duration: number, success: boolean, options?: {
    errorType?: string
    cacheHit?: boolean
    requestSize?: number
    responseSize?: number
  ...
  getPerformanceSummary(): {
    ...
    const cacheEfficiency = this.metrics.totalCacheRequests > 0
    ...
    if (successRate < 95) {
      ...
      overallHealth = 'fair'
      recommendations.push('Moderate error rate. Consider improving error handling.')
    }

    if (this.metrics.averageResponseTime > 2000) {
      overallHealth = overallHealth === 'excellent' ? 'fair' : 'poor'
      recommendations.push('High response times detected. Consider optimizing API calls or caching.')
    } else if (this.metrics.averageResponseTime > 1000) {
      if (overallHealth === 'excellent') overallHealth = 'good'
      recommendations.push('Response times could be improved with better caching or optimization.')
    }
  ...
  private updateMetricsFromEvent(event: PerformanceEvent): void {
    ...
    if (event.success) {
      ...
      if (event.errorType) {
        this.metrics.errorsByType[event.errorType] = (this.metrics.errorsByType[event.errorType] || 0) + 1
      }
      this.metrics.errorsByOperation[event.operation] = (this.metrics.errorsByOperation[event.operation] || 0) + 1
    }
  ...
  private updateCacheMetrics(cacheHit: boolean, duration: number): void {
    ...
    if (cacheHit) {
      ...
      const hitCount = Math.round(this.metrics.cacheHitRate * (this.metrics.totalCacheRequests - 1) / 100) + 1
      this.metrics.cacheHitRate = (hitCount / this.metrics.totalCacheRequests) * 100
    }
    ...
    this.metrics.averageCacheResponseTime = 
      (this.metrics.averageCacheResponseTime * (this.metrics.totalCacheRequests - 1) + duration) / this.metrics.totalCacheRequests
  }
  ...
  private updateOperationMetrics(event: PerformanceEvent): void {
    ...
    if (opMetrics) {
      ...
      opMetrics.averageResponseTime = 
        opMetrics.responseTimes.reduce((sum, time) => sum + time, 0) / opMetrics.responseTimes.length
    }
  }
  ...
  private updateMetricsFromEventForAggregation(metrics: PerformanceMetrics, event: PerformanceEvent): void {
    ...
    if (event.success) {
      ...
      metrics.errorsByOperation[event.operation] = (metrics.errorsByOperation[event.operation] || 0) + 1
    }
    ...
    if (event.cacheHit !== undefined) {
      ...
      if (event.cacheHit) {
        const hitCount = Math.round(metrics.cacheHitRate * (metrics.totalCacheRequests - 1) / 100) + 1
      ...
      metrics.averageCacheResponseTime = 
        (metrics.averageCacheResponseTime * (metrics.totalCacheRequests - 1) + event.duration) / metrics.totalCacheRequests
    }
