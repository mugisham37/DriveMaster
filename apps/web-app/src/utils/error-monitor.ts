...
export class ErrorMonitor {
    ...
    if (filter) {
      if (filter.operation) {
        filteredErrors = filteredErrors.filter(
          (e) => e.operation === filter.operation,
        );
      }
      if (filter.errorType) {
        filteredErrors = filteredErrors.filter(
          (e) => e.error.type === filter.errorType,
        );
      }
      if (filter.severity) {
        filteredErrors = filteredErrors.filter(
  ...
  getErrorSummary(): {
    ...
    if (errorRate > 10) {
      ...
      overallHealth = "fair";
      recommendations.push(
        "Elevated error rate. Monitor closely and investigate root causes.",
      );
    } else if (errorRate > 2) {
      overallHealth = "good";
      recommendations.push(
        "Moderate error rate. Consider preventive measures.",
      );
    }
    ...
    if (criticalErrors > 5) {
      overallHealth = "poor";
      recommendations.push(
        "Multiple critical errors detected. Escalate immediately.",
      );
    } else if (criticalErrors > 0) {
      if (overallHealth === "excellent") overallHealth = "good";
      recommendations.push(
        "Critical errors present. Review and resolve promptly.",
      );
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        "Error rates are within acceptable limits. Continue monitoring.",
      );
    }
  ...
  private setupDefaultAlertRules(): void {
    ...
    this.addAlertRule({
      ...
      condition: (metrics) =>
        metrics.criticalErrors > this.config.alertThresholds.criticalErrorCount,
      severity: "critical",
    ...
    this.addAlertRule({
      ...
      condition: (metrics) =>
        metrics.averageErrorFrequency >
        this.config.alertThresholds.errorFrequency,
      severity: "warning",
  ...
  private getMostFrequentErrorType(): ContentServiceErrorType {
    ...
    return errorTypes.reduce((a, b) =>
      a[1] > b[1] ? a : b,
    )[0] as ContentServiceErrorType;
  }
  ...
  private getThresholdForRule(rule: AlertRule): number {
    ...
    if (rule.name === "Critical Errors")
      return this.config.alertThresholds.criticalErrorCount;
    if (rule.name === "High Error Frequency")
      return this.config.alertThresholds.errorFrequency;
    return 0;
  }
  ...
  private getCurrentValueForRule(rule: AlertRule): number {
    ...
    if (rule.name === "High Error Frequency")
      return this.metrics.averageErrorFrequency;
    return 0;
  }
  ...
  private analyzeTrends(): void {
    ...
    const windowStart = new Date(
      now.getTime() - this.config.trendAnalysisInterval,
    );
    ...
    for (const error of windowErrors) {
      errorTypeCounts[error.error.type] =
        (errorTypeCounts[error.error.type] || 0) + 1;
    }

    const dominantErrorType = Object.entries(errorTypeCounts).reduce(
      (a, b) => (a[1] > b[1] ? a : b),
      ["server", 0],
    )[0] as ContentServiceErrorType;
    ...
    // Determine trend
    const previousTrend =
      this.metrics.errorTrends[this.metrics.errorTrends.length - 1];
  ...
  private getTotalRequestsInWindow(start: Date, end: Date): number {
    ...
    return windowErrors.length > 0
      ? Math.round(windowErrors.length / estimatedErrorRate)
      : 0;
  }
