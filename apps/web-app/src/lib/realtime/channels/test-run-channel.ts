...
export class TestRunChannel {
  ...
  private async subscribe(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Subscribed to test run channel: ${this.testRun.submissionUuid}`,
      );
    } catch (error) {
  ...
  disconnect(): void {
    ...
    console.log(
      `Disconnected from test run channel: ${this.testRun.submissionUuid}`,
    );
  }
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on("connected", () => {
      console.log(
        `Test run channel connection established: ${this.testRun.submissionUuid}`,
      );
    });

    this.connection.on("disconnected", () => {
      console.log(
        `Test run channel connection lost: ${this.testRun.submissionUuid}`,
      );
    });

    this.connection.on("error", (event) => {
      console.error(
        `Test run channel error: ${this.testRun.submissionUuid}`,
        event.error,
      );
    });
  }
  ...
  private handleMessage(data: Record<string, unknown>): void {
    try {
      ...
      if (data.type === "confirm_subscription") {
        console.log(
          `Test run channel subscription confirmed: ${this.testRun.submissionUuid}`,
        );
        return;
      }
      ...
      if (data.type === "reject_subscription") {
        console.error(
          `Test run channel subscription rejected: ${this.testRun.submissionUuid}`,
        );
        return;
      }
