...
export type MentorRequestEventHandler = (
...
export class MentorRequestChannel {
  ...
  disconnect(): void {
    ...
    console.log(
      `Disconnected from mentor request channel: ${this.request.uuid}`,
    );
  }
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on("connected", () => {
      console.log(
        `Mentor request channel connection established: ${this.request.uuid}`,
      );
    });

    this.connection.on("disconnected", () => {
      console.log(
        `Mentor request channel connection lost: ${this.request.uuid}`,
      );
    });

    this.connection.on("error", (event) => {
      console.error(
        `Mentor request channel error: ${this.request.uuid}`,
        event.error,
      );
    });
  }
  ...
  private handleMessage(data: Record<string, unknown>): void {
    try {
      ...
      if (data.type === "confirm_subscription") {
        console.log(
          `Mentor request channel subscription confirmed: ${this.request.uuid}`,
        );
        return;
      }
      ...
      if (data.type === "reject_subscription") {
        console.error(
          `Mentor request channel subscription rejected: ${this.request.uuid}`,
        );
        return;
      }
      ...
      if (data.message) {
        const response = camelizeKeysAs<MentorRequestChannelResponse>(
  ...
  private isValidMentorRequestResponse(
    data: unknown,
  ): data is MentorRequestChannelResponse {
    return (
      data !== null &&
      data !== undefined &&
      typeof data === "object" &&
      ...
      "uuid" in
        ((data as Record<string, unknown>).mentorRequest as Record<
          string,
          unknown
        >) &&
      "status" in
      ...
      ["cancelled", "pending", "fulfilled"].includes(
        (
          (data as Record<string, unknown>).mentorRequest as Record<
            string,
            unknown
