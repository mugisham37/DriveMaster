...
export type SolutionWithLatestIterationEventHandler = (
...
export class SolutionWithLatestIterationChannel {
  ...
  private async subscribe(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Subscribed to solution with latest iteration channel: ${this.solution.uuid}`,
      );
    } catch (error) {
      console.error(
        "Failed to subscribe to solution with latest iteration channel:",
        error,
      );
      throw error;
    }
  ...
  disconnect(): void {
    ...
    console.log(
      `Disconnected from solution with latest iteration channel: ${this.solution.uuid}`,
    );
  }
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on("connected", () => {
      console.log(
        `Solution with latest iteration channel connection established: ${this.solution.uuid}`,
      );
    });

    this.connection.on("disconnected", () => {
      console.log(
        `Solution with latest iteration channel connection lost: ${this.solution.uuid}`,
      );
    });

    this.connection.on("error", (event) => {
      console.error(
        `Solution with latest iteration channel error: ${this.solution.uuid}`,
        event.error,
      );
    });
  }
  ...
  private handleMessage(data: Record<string, unknown>): void {
    try {
      ...
      if (data.type === "confirm_subscription") {
        console.log(
          `Solution with latest iteration channel subscription confirmed: ${this.solution.uuid}`,
        );
        return;
      }
      ...
      if (data.type === "reject_subscription") {
        console.error(
          `Solution with latest iteration channel subscription rejected: ${this.solution.uuid}`,
        );
        return;
      }
      ...
      if (data.message) {
        const response = camelizeKeys(
          data.message,
        ) as SolutionWithLatestIterationResponse;
        ...
        if (this.isValidResponse(response)) {
          ...
          console.error(
            "Invalid solution with latest iteration response received:",
            response,
          );
        }
      }
      ...
      console.error(
        "Error handling solution with latest iteration message:",
        error,
      );
    }
  }
  ...
  private isValidResponse(
    data: unknown,
  ): data is SolutionWithLatestIterationResponse {
