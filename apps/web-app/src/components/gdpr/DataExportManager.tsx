...
export function DataExportManager({
  ...
  const canRequestExport = !activeExport || 
    (activeExport.status === 'completed' || activeExport.status === 'failed')
  ...
  const renderExportStatus = (exportRequest: GDPRExportResponse) => {
    ...
    const isExpired = exportRequest.expiresAt && new Date() > new Date(exportRequest.expiresAt)
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 text-sm">
              <div className={`text-${config.color}-700`}>
                <span className="font-medium">Request ID:</span> {exportRequest.requestId}
              </div>
              <div className={`text-${config.color}-700`}>
            ...
            {isExpired && exportRequest.status === 'completed' && (
              <div className="mt-3 p-3 bg-red-100 border border-red-200 rounded-lg">
                <p className="text-red-800 text-sm">
                  ...
                  Please request a new export if you still need your data.
                </p>
              </div>
            )}
          </div>
  ...
  const renderExportInfo = () => (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
      ...
      <div className="mt-6 p-4 bg-blue-100 rounded-lg">
        <h4 className="font-medium text-blue-800 mb-2">Export Format & Security</h4>
        <ul className="text-sm text-blue-700 space-y-1">
    ...
    return (
      <div className="space-y-4">
        {exportHistory.map((exportRequest) => (
          <div key={exportRequest.requestId} className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Requested: {new Date(exportRequest.createdAt).toLocaleString()}
                </p>
                {exportRequest.expiresAt && (
                  <p className="text-xs text-gray-500">
                    Expires: {new Date(exportRequest.expiresAt).toLocaleString()}
                  </p>
                )}
              </div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="text-gray-600 mb-4">
                Get a complete copy of all your personal data stored in our system. 
                This includes your profile, learning progress, activity history, and privacy settings.
              </p>
        ...
        <div className="flex items-start">
          ...
          <div className="text-sm text-gray-700">
            ...
            <ul className="space-y-1">
              <li>• Export requests are processed within 30 days as required by GDPR</li>
              <li>• Download links are valid for 7 days after completion</li>
              <li>• You can request a new export at any time</li>
              <li>• All export requests are logged for security and compliance</li>
              <li>• Exported data includes all information we have about you at the time of export</li>
            </ul>
          </div>
        </div>
      </div>
