...
function ErrorIcon({
  ...
  switch (type) {
    case "network":
      return (
        <svg
          className={`${iconClasses} text-orange-400`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          {showRecovery && recoverySuggestions.length > 0 && (
            <div className="mt-3">
              ...
              <ul
                className={`mt-1 text-sm ${styling.message} list-disc list-inside space-y-1`}
              >
            ...
            {authError.recoverable && onRetry && (
              ...
              >
                {authError.retryAfter
                  ? `Try Again (${authError.retryAfter}s)`
                  : "Try Again"}
              </button>
            )}
...
export function useAuthErrorDisplay() {
  ...
  const showError = React.useCallback(
    (error: AuthError | Error | unknown, context?: string) => {
      const authError =
        error instanceof Error && !("type" in error)
          ? AuthErrorHandler.processError(error, context)
          : (error as AuthError);
