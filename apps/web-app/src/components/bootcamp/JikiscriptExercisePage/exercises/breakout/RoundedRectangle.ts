...
function fn(exercise: BreakoutExercise) {
  const drawRectangle = (
    ...
    div.style.backgroundColor = rectangle.getUnwrappedField(
      "fill_color_hex"
    ) as string;
    div.style.borderRadius = `${rectangle.getUnwrappedField(
      "corner_radius"
    )}px`;
    div.style.opacity = "0";
    ...
    exercise.animateIntoView(
      executionCtx,
      `#${exercise.getView().id} #rect-${rectangle.objectId}`
    );
  ...
  const move = (executionCtx: ExecutionContext, rectangle: Jiki.Instance) => {
    exercise.addAnimation({
      targets: `#${exercise.getView().id} #rect-${rectangle.objectId}`,
      duration: 1,
      transformations: {
...
export function buildRectangle(exercise: BreakoutExercise) {
  return fn(exercise);
}
