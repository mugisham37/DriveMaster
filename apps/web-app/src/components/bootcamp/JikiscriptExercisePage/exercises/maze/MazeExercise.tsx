...
export default class MazeExercise extends Exercise {
  ...
  private startingAngles: Record<string, number> = {
    down: 180,
    up: 0,
    left: -90,
    right: 90,
  ...
  public randomEmojisAllCollected() {
    ...
    const expected = this.randomEmojis.reduce(
  ...
  public setupPosition(_executionCtx: ExecutionContext, x: number, y: number) {
    ...
    this.character.style.left = `${
      this.characterPosition.x * this.squareSize
    }%`;
    this.character.style.top = `${this.characterPosition.y * this.squareSize}%`;
  }
  public announceEmojis(
    _executionCtx: ExecutionContext,
    emojis: Jiki.JikiDictionary
  ) {
  ...
  public override availableFunctions = [
    ...
    },
    {
      name: "can_turn_left",
      func: (...args: unknown[]) =>
        this.canTurnLeft(args[0] as ExecutionContext),
      description: "checked if the character can turn left",
    },
    {
      name: "can_turn_right",
      func: (...args: unknown[]) =>
        this.canTurnRight(args[0] as ExecutionContext),
    ...
    },
    {
      name: "look",
      func: (...args: unknown[]) =>
        this.look(args[0] as ExecutionContext, args[1] as Jiki.JikiString),
      description: "looked in a direction and returns what is there",
    },
    {
      name: "get_initial_maze",
      func: (...args: unknown[]) =>
        this.getInitialMaze(args[0] as ExecutionContext),
    ...
    },
    {
      name: "remove_emoji",
      func: (...args: unknown[]) =>
        this.removeEmoji(args[0] as ExecutionContext),
