...
import { useContentItems, useContentFilters, useContentPagination } from '../../hooks/use-content-operations'
import { useWorkflowStatusSummary } from '../../hooks/use-workflow-operations'
import type { ContentItem, QueryItemsDto, WorkflowStatus, ContentType } from '../../types'
...
export function ContentListItem({
  ...
  return (
    <div className={`bg-white border rounded-lg p-4 hover:shadow-md transition-shadow ${isSelected ? 'ring-2 ring-blue-500' : ''}`}>
...
function ContentFilters({ filters, onFiltersChange, onReset }: ContentFiltersProps) {
  return (
    <div className="bg-white border rounded-lg p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select
            value={filters.type || ''}
            onChange={(e) => onFiltersChange({ type: e.target.value as ContentType || undefined })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => onFiltersChange({ status: e.target.value as WorkflowStatus || undefined })}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
  ...
  return (
    <div className="bg-white border rounded-lg p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="text-center">
          <div className="text-2xl font-bold text-blue-600">{summary.published}</div>
          <div className="text-sm text-gray-500">Published</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-red-600">{summary.archived}</div>
...
export function ContentList({
  ...
  // Hooks
  const { params, updateFilters, resetFilters } = useContentFilters(initialFilters)
  ...
  const { items, total, totalPages, isLoading, error, refresh } = useContentItems(queryParams)
  ...
  const isAllSelected = items.length > 0 && localSelectedItems.length === items.length
  const isPartiallySelected = localSelectedItems.length > 0 && localSelectedItems.length < items.length
          ...
          <div className="p-4 space-y-4">
            {items.map((item) => (
              <ContentListItem
                ...
                onSelect={selectable ? () => handleItemSelect(item.id, !localSelectedItems.includes(item.id)) : onItemSelect ? () => onItemSelect(item) : undefined}
