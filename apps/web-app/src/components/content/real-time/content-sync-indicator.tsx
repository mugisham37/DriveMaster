...
import { useContentSync, useWebSocketConnection } from '../../hooks/use-real-time-content'
      ...
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="mb-6">
          <p className="text-gray-600 mb-4">
            The content has been modified by another user while you were editing. 
          ...
          {conflictDetails && (
            <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{conflictDetails.localChanges}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Remote changes:</span>
                <span className="font-medium">{conflictDetails.remoteChanges}</span>
              </div>
              <div className="flex justify-between">
...
export function ContentSyncIndicator({
  ...
  const getSyncStatus = (): 'synced' | 'syncing' | 'conflict' | 'error' | 'offline' => {
  ...
  const handleConflictResolve = async (strategy: 'local_wins' | 'remote_wins' | 'merge') => {
    try {
...
export function ConnectionStatusIndicator({
  ...
  const { isConnected, connectionState, connectionStats } = useWebSocketConnection()
...
export function RealTimeStatusBar({
  ...
  return (
    <div className={`flex items-center justify-between p-2 bg-gray-50 border-t text-sm ${className}`}>
