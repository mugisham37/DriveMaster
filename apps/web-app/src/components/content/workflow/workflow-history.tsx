...
export function WorkflowTransitionCard({
  ...
  if (compact) {
    return (
      <div className="flex items-center space-x-3 py-2">
        <div
          className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${config.bgColor} ${config.borderColor} border`}
        >
  ...
  return (
    <div
      className={`border rounded-lg p-4 ${config.bgColor} ${config.borderColor} ${isLatest ? "ring-2 ring-blue-500 ring-opacity-20" : ""}`}
    >
      <div className="flex items-start space-x-3">
        <div
          className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${config.bgColor} ${config.borderColor} border-2`}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          {transition.metadata && (
            <div className="mt-2 text-sm text-gray-600">
              ...
              {transition.metadata.fromStatus &&
                transition.metadata.toStatus && (
                  <div>
...
function filterTransitions(
  transitions: ExtendedWorkflowTransition[],
  filters: HistoryFilters,
): ExtendedWorkflowTransition[] {
    ...
    return (
      <div className={`text-center p-6 ${className}`}>
        ...
        <p className="text-sm text-gray-500">
          Workflow history will appear here as actions are performed on this
          content.
        </p>
      </div>
    );
