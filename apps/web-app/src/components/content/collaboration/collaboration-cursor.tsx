...
export function RemoteSelection({ user, selection, color }: RemoteSelectionProps) {
  return (
    <div
...
export function CollaborationCursor({
  ...
  const [localCursorPosition, setLocalCursorPosition] = useState<{ line: number; column: number } | null>(null)
  const [localSelection, setLocalSelection] = useState<{ start: number; end: number; text: string } | null>(null)
  const lastSentPosition = useRef<{ line: number; column: number } | null>(null)
  const lastSentSelection = useRef<{ start: number; end: number; text: string } | null>(null)
  ...
  }, [enabled, editorRef, sendCursorPosition, sendTextSelection, onCursorMove, onTextSelect])
    ...
    <div className={`${className}`}>
      <div className="flex items-center space-x-2">
        ...
        <span className="text-sm font-medium text-gray-700">
          Collaborating with {participantCount} user{participantCount !== 1 ? 's' : ''}
        </span>
      </div>
 ...
 */
function calculateCursorPosition(range: Range, container: HTMLElement): { line: number; column: number } | null {
  try {
