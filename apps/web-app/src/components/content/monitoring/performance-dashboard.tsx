  ...
  useEffect(() => {
    const unsubscribePerformance = performanceMonitor.subscribe((metrics) => {
      ...
      setHistoricalData(prev => ({
        responseTime: [...prev.responseTime.slice(-19), { timestamp: now, value: metrics.averageResponseTime }],
        errorRate: [...prev.errorRate.slice(-19), { timestamp: now, value: metrics.errorRate }],
        cacheHitRate: [...prev.cacheHitRate.slice(-19), { timestamp: now, value: metrics.cacheHitRate }]
      }))
    })
    ...
    return (
      <div className={`p-6 ${className}`}>
        <div className="text-center text-gray-500">
          ...
          <p className="text-sm">Metrics will appear once content service operations begin</p>
        </div>
      </div>
    )
  }
    ...
    <div className={`p-6 space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <div className={`w-3 h-3 rounded-full ${
            performanceSummary?.overallHealth === 'excellent' ? 'bg-green-500' :
            performanceSummary?.overallHealth === 'good' ? 'bg-blue-500' :
            performanceSummary?.overallHealth === 'fair' ? 'bg-yellow-500' : 'bg-red-500'
          }`}></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          unit="%"
          status={
            (performanceSummary?.successRate || 0) >= 99 ? 'excellent' :
            (performanceSummary?.successRate || 0) >= 95 ? 'good' :
            (performanceSummary?.successRate || 0) >= 90 ? 'warning' : 'critical'
          }
        ...
        <MetricCard
          ...
          unit="ms"
          status={
            performanceMetrics.averageResponseTime <= 500 ? 'excellent' :
            performanceMetrics.averageResponseTime <= 1000 ? 'good' :
            performanceMetrics.averageResponseTime <= 2000 ? 'warning' : 'critical'
          }
        ...
        <MetricCard
          ...
          unit="%"
          status={
            performanceMetrics.cacheHitRate >= 80 ? 'excellent' :
            performanceMetrics.cacheHitRate >= 60 ? 'good' :
            performanceMetrics.cacheHitRate >= 40 ? 'warning' : 'critical'
          }
        ...
        <MetricCard
          ...
          unit="%"
          status={
            errorMetrics.errorRate <= 1 ? 'excellent' :
            errorMetrics.errorRate <= 3 ? 'good' :
                  ...
                  <div key={operation} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {metrics.totalCalls} calls, {metrics.successfulCalls} successful
                      </div>
                    </div>
      ...
      <div className="text-xs text-gray-500 text-center">
        ...
        Window: {Math.round(performanceMetrics.metricsWindow.duration / 60000)} minutes • 
