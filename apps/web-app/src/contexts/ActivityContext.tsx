...
export type ActivityAction =
  ...
  | { type: 'INSIGHTS_FETCH_SUCCESS'; payload: { insights: ActivityInsight[]; recommendations: ActivityRecommendation[] } }
  ...
  | { type: 'SESSION_END'; payload: { sessionId: string; endTime: Date; duration: number } }
  | { type: 'SESSION_UPDATE'; payload: { sessionId: string; activity: ActivityRecord } }
  
  // Analytics actions
  | { type: 'BEHAVIOR_PATTERNS_UPDATE'; payload: { patterns: BehaviorPattern[] } }
  | { type: 'TOPIC_ACTIVITIES_UPDATE'; payload: { topicActivities: TopicActivitySummary[] } }
  ...
  // Error management
  | { type: 'CLEAR_ERROR'; payload?: { errorType?: keyof Pick<ActivityState, 'error' | 'summaryError' | 'metricsError' | 'insightsError'> } }
  ...
  switch (action.type) {
    ...
    case 'ACTIVITY_RECORD_SUCCESS':
      return {
        ...state,
        isRecording: false,
        recentActivities: [action.payload.activity, ...state.recentActivities].slice(0, 50),
    ...
    case 'ACTIVITY_BATCH_ADD':
      return {
        ...state,
        pendingActivities: [...state.pendingActivities, action.payload.activity],
      }
    ...
    case 'ACTIVITY_BATCH_FLUSH':
      return {
        ...state,
        pendingActivities: [],
        recentActivities: [...action.payload.activities, ...state.recentActivities].slice(0, 50),
    ...
    case 'SESSION_START':
      return {
        ...
        sessionHistory: [action.payload.session, ...state.sessionHistory].slice(0, 10),
      }
    ...
    case 'SESSION_END':
      ...
      return {
        ...
        sessionHistory: updatedSession ? 
          [updatedSession, ...state.sessionHistory.filter(s => s.id !== action.payload.sessionId)].slice(0, 10) :
    ...
    case 'SESSION_UPDATE':
      ...
      return {
        ...
        currentSession: {
          ...state.currentSession,
          activities: [...state.currentSession.activities, action.payload.activity],
        },
      }
...
export interface ActivityContextValue {
  ...
  // Activity recording
  recordActivity: (activityType: ActivityType, metadata?: Record<string, unknown>) => Promise<void>
  recordBatchActivities: (activities: Omit<ActivityRecord, 'id' | 'timestamp'>[]) => Promise<void>
  ...
  // Utility functions
  clearError: (errorType?: keyof Pick<ActivityState, 'error' | 'summaryError' | 'metricsError' | 'insightsError'>) => void
  clearAllErrors: () => void
...
export function ActivityProvider({ children }: ActivityProviderProps) {
  ...
  const activityInsightsQuery = useQuery<{ insights: ActivityInsight[]; recommendations: ActivityRecommendation[] }, UserServiceError>({
    ...
    enabled: !!userId && isAuthenticated,
    ...createUserServiceQueryOptions<{ insights: ActivityInsight[]; recommendations: ActivityRecommendation[] }, UserServiceError>(
  ...
  }, [state.batchingEnabled, state.pendingActivities.length, flushPendingActivities])
  ...
  }, [userId, state.currentSession, state.batchingEnabled, recordActivityMutation])
  ...
  const fetchActivitySummary = useCallback(async (dateRange?: DateRange) => {
    if (!userId) return
    
    try {
      if (dateRange) {
        // Fetch with custom date range
        const summary = await userServiceClient.getActivitySummary(userId, dateRange)
  ...
  const fetchEngagementMetrics = useCallback(async (dateRange?: DateRange) => {
    if (!userId) return
    
    try {
      if (dateRange) {
        // Fetch with custom date range
        const metrics = await userServiceClient.getEngagementMetrics(userId, dateRange)
  ...
  const calculateEngagementTrends = useCallback(async (days: number): Promise<EngagementTrend[]> => {
    if (!userId || !state.summary) return []
    
    try {
      // Import analytics utilities
      const { ActivityAnalyticsManager } = await import('@/lib/user-service/activity-analytics')
  ...
  const detectBehaviorPatterns = useCallback(async (): Promise<BehaviorPattern[]> => {
    if (!userId || !state.summary) return []
    
    try {
      const { BehaviorPatternAnalyzer } = await import('@/lib/user-service/activity-analytics')
  ...
  const generatePersonalizedInsights = useCallback(async (): Promise<ActivityInsight[]> => {
    if (!userId || !state.summary) return []
    
    try {
      const { PersonalizedInsightsGenerator } = await import('@/lib/user-service/activity-analytics')
  ...
  const getActivityRecommendations = useCallback(async (): Promise<ActivityRecommendation[]> => {
    if (!userId || !state.summary) return []
    
    try {
      const { ActivityRecommendationEngine } = await import('@/lib/user-service/activity-analytics')
      
      const recommendations = await ActivityRecommendationEngine.generateRecommendations(
        userId,
        state.summary,
        state.insights,
        state.behaviorPatterns
  ...
  const generateActivityReport = useCallback(async (dateRange: DateRange): Promise<ActivityReport> => {
    if (!userId) {
      throw new Error('User not authenticated')
    }
    
    try {
      const { ActivityReportGenerator } = await import('@/lib/user-service/activity-analytics')
  ...
  const getUsageAnalytics = useCallback(async (): Promise<UsageAnalytics> => {
    ...
    try {
      const { UsageAnalyticsCalculator } = await import('@/lib/user-service/activity-analytics')
  ...
  const exportActivityData = useCallback(async (format: 'json' | 'csv'): Promise<string> => {
    if (!userId || !state.summary) {
      throw new Error('No activity data to export')
    }
    
    try {
      const { ActivityDataExporter } = await import('@/lib/user-service/activity-analytics')
  ...
  }, [userId, state.summary, state.recentActivities, state.sessionHistory, state.engagementMetrics])
  
  // ============================================================================
  // Performance Optimization Functions (Task 7.4)
  // ============================================================================
  
  const enableBatching = useCallback((enabled: boolean) => {
  ...
  const clearError = useCallback((errorType?: keyof Pick<ActivityState, 'error' | 'summaryError' | 'metricsError' | 'insightsError'>) => {
    if (errorType) {
  ...
  const getActivityCount = useCallback((activityType?: ActivityType): number => {
    if (!activityType) {
      return state.recentActivities.length
    }
    return state.recentActivities.filter(a => a.activityType === activityType).length
  }, [state.recentActivities])
  
  const getSessionStats = useCallback((): SessionStats => {
    const sessions = [state.currentSession, ...state.sessionHistory].filter(Boolean) as ActivitySession[]
  ...
  const contextValue: ActivityContextValue = {
    ...
    // Computed properties
    summary: state.summary || (activitySummaryQuery.data as ActivitySummary) || null,
    engagementMetrics: state.engagementMetrics || (engagementMetricsQuery.data as EngagementMetrics) || null,
    insights: state.insights,
    recommendations: state.recommendations,
    currentSession: state.currentSession,
    isLoading: state.isLoading || activitySummaryQuery.isLoading || engagementMetricsQuery.isLoading || activityInsightsQuery.isLoading,
    isRecording: state.isRecording || recordActivityMutation.isPending,
    error: state.error || state.summaryError || state.metricsError || state.insightsError,
