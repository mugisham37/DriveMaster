...
export function AnalyticsProvider({ 
  ...
  }, [autoConnect, isAuthenticated, enableRealtime, connect, checkServiceHealth, isInitialized, isInitializing])
  ...
  useEffect(() => {
    ...
    if (typeof webSocketManager.addEventListener === 'function') {
      webSocketManager.addEventListener('connection_status_changed', handleConnectionChange)
      webSocketManager.addEventListener('error', handleError)
    }
    
    return () => {
      if (typeof webSocketManager.removeEventListener === 'function') {
        webSocketManager.removeEventListener('connection_status_changed', handleConnectionChange)
...
export function useAnalyticsConnection(): {
  ...
  const { isConnected, connectionStatus, connect, disconnect, reconnect } = useAnalyticsContext()
