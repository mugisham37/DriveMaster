...
} from "@/lib/auth/resilience-integration";
import {
  optimizedAuthOps,
...
export type AuthAction =
  ...
  | {
      ...
      payload: { user: UserProfile; tokens: TokenPair };
    }
  ...
  | {
      type: "OAUTH_START";
      payload: { provider: OAuthProviderType; redirectUrl?: string };
    }
  ...
  | {
      type: "SESSION_UPDATE";
      payload: { lastActivity: Date; tokenExpiration: Date | null };
    }
...
export interface AuthContextValue {
  ...
  clearError: (
    errorType?: keyof Pick<
      AuthState,
      "error" | "loginError" | "registerError" | "profileError" | "oauthError"
    >,
  ) => void;
  clearAllErrors: () => void;
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      try {
          ...
          try {
            ...
            if (isMounted && profileResult.data) {
              ...
              if (profileResult.degraded) {
                console.warn(
                  "Authentication initialized in degraded mode:",
            ...
            if (!failureResult.shouldLogout) {
              console.warn(
                "Profile fetch failed during initialization, continuing without user data:",
                failureResult.technicalMessage,
              );
            }
          }
        }
        ...
        if (isMounted) {
          const authError = AuthErrorHandler.processError(
            error,
            "initialization",
          );
          dispatch({
  ...
  const login = useStableAuthCallback(
    async (credentials: LoginCredentials): Promise<void> => {
      dispatch({ type: "LOGIN_START" });

      try {
        // Use optimized login with caching and deduplication
        const { tokens, user }: { tokens: TokenPair; user: UserProfile } =
          await optimizedAuthOps.login(
            credentials,
        ...
        dispatch({
          type: "LOGIN_ERROR",
          payload: {
            error: { ...authError, message: errorClassification.userMessage },
          },
        });
  ...
  const register = useStableAuthCallback(
    async (userData: RegisterData): Promise<void> => {
      ...
      try {
        ...
        const { tokens, user }: { tokens: TokenPair; user: UserProfile } =
          await optimizedAuthOps.register(
            userData,
  ...
  const logout = useStableAuthCallback(async (): Promise<void> => {
    ...
    try {
      ...
      await authResilience.executeWithResilience(() => authClient.logout(), {
        ...
        dispatch({
          ...
          payload: {
            tokens: {
              ...
              expiresIn:
                Math.floor(
                  (tokenInfo.accessTokenExpiration?.getTime() || Date.now()) -
                    Date.now(),
                ) / 1000,
            },
          },
  ...
  const initiateOAuth = useStableAuthCallback(
    async (
      provider: OAuthProviderType,
      redirectUrl?: string,
    ): Promise<void> => {
      ...
      try {
        ...
        const response: unknown = await optimizedAuthOps.initiateOAuth(
          provider,
          redirectUrl,
          (prov: OAuthProviderType, redirUrl?: string) =>
            authClient.initiateOAuth(prov, redirUrl),
        );

        if (
          response &&
          typeof response === "object" &&
          "authorizationUrl" in response
        ) {
    ...
    try {
      ...
      const user: UserProfile = await optimizedAuthOps.getProfile(
        async (): Promise<UserProfile> => {
          ...
          if (profileResult.degraded) {
            console.info(
              "Profile loaded from cache/fallback:",
              profileResult.source,
            );
          }
      ...
      if (!failureResult.shouldLogout) {
        console.warn(
          "Profile fetch failed, continuing with existing data:",
          failureResult.technicalMessage,
        );
      }
  ...
  const updateProfile = useStableAuthCallback(
  ...
  const clearError = useStableAuthCallback(
    (
      errorType?: keyof Pick<
        AuthState,
        "error" | "loginError" | "registerError" | "profileError" | "oauthError"
