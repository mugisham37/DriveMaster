...
export type GDPRAction =
  ...
  | {
      ...
      payload: {
        ...
        historyEntry: ConsentHistoryEntry;
      };
    }
  ...
  | {
      ...
      payload: { exportResponse: GDPRExportResponse };
    }
  | { type: "EXPORT_REQUEST_ERROR"; payload: { error: UserServiceError } }
  | {
      type: "EXPORT_STATUS_UPDATE";
      payload: { requestId: string; status: GDPRExportResponse };
    }
  ...
  | {
      ...
      payload: { deleteResponse: GDPRDeleteResponse };
    }
  | { type: "DELETE_REQUEST_ERROR"; payload: { error: UserServiceError } }
  | {
      type: "DELETE_STATUS_UPDATE";
      payload: { requestId: string; status: GDPRDeleteResponse };
    }
  ...
  | {
      ...
      payload: { retention: DataRetentionPreference };
    }
...
function gdprReducer(state: GDPRState, action: GDPRAction): GDPRState {
  switch (action.type) {
    ...
    case "EXPORT_REQUEST_SUCCESS":
      ...
      updatedExportRequests.set(
        action.payload.exportResponse.requestId,
        action.payload.exportResponse,
      );
      return {
        ...state,
        isExportLoading: false,
    ...
    case "EXPORT_STATUS_UPDATE":
      ...
      return {
        ...
        exportRequests,
        activeExportRequest:
          state.activeExportRequest?.requestId === action.payload.requestId
    ...
    case "DELETE_REQUEST_SUCCESS":
      ...
      updatedDeleteRequests.set(
        action.payload.deleteResponse.requestId,
    ...
    case "DELETE_STATUS_UPDATE":
      ...
      return {
        ...
        deleteRequests,
        activeDeleteRequest:
          state.activeDeleteRequest?.requestId === action.payload.requestId
            ? action.payload.status
    ...
    case "PRIVACY_ALERT_ADD":
      return {
        ...state,
        privacyAlerts: [action.payload.alert, ...state.privacyAlerts].slice(
          0,
          20,
        ),
      };
...
export interface GDPRContextValue {
  ...
  clearError: (
    errorType?: keyof Pick<
      GDPRState,
      "error" | "consentError" | "exportError" | "deleteError" | "reportError"
    >,
  ) => void;
  clearAllErrors: () => void;
...
export function GDPRProvider({ children }: GDPRProviderProps) {
  ...
  const consentUpdateMutation = useMutation({
    mutationFn: async (consent: Partial<ConsentPreferences>) => {
      ...
      await userServiceClient.updateConsent(
        userId,
        consent as ConsentPreferences,
      );
      return consent;
    },
    onMutate: () => {
  ...
  const deleteDataMutation = useMutation({
    ...
    onSuccess: (deleteResponse) => {
      dispatch({
        type: "DELETE_REQUEST_SUCCESS",
        payload: {
          deleteResponse: deleteResponse as unknown as GDPRDeleteResponse,
        },
      });
    },
    onError: (error: UserServiceError) => {
  ...
  const updateConsentPreferences = useCallback(
    async (consent: Partial<ConsentPreferences>) => {
  ...
  const grantConsent = useCallback(
    async (consentType: keyof ConsentPreferences, purpose: string) => {
  ...
  const withdrawConsent = useCallback(
    async (consentType: keyof ConsentPreferences, reason: string) => {
  ...
  const requestDataExport =
    useCallback(async (): Promise<GDPRExportResponse> => {
  ...
  const checkExportStatus = useCallback(
    async (requestId: string): Promise<GDPRExportResponse> => {
      if (!userId) throw new Error("User not authenticated");
  ...
  const requestDataDeletion = useCallback(
    async (reason: string): Promise<GDPRDeleteResponse> => {
      ...
      const auditEntry: AuditLogEntry = {
        ...
        details: {
          reason,
          requestId: (response as unknown as { requestId: string }).requestId,
        },
      };
  ...
  const checkDeletionStatus = useCallback(
    async (requestId: string): Promise<GDPRDeleteResponse> => {
  ...
  const generatePrivacyReport =
    useCallback(async (): Promise<PrivacyReport> => {
      if (!userId) throw new Error("User not authenticated");
  ...
  const getAuditLog = useCallback(
    async (days: number = 30): Promise<AuditLogEntry[]> => {
  ...
  const runComplianceCheck = useCallback(async (): Promise<
    ComplianceCheck[]
  > => {
    ...
    const checks: ComplianceCheck[] = [
      {
        ...
        recommendation: state.consentPreferences
  ...
  const exerciseUserRight = useCallback(
    async (
      rightType: RightsExerciseEntry["rightType"],
      details: Record<string, unknown>,
    ): Promise<void> => {
  ...
  const updatePrivacySettings = useCallback(
    async (settings: Partial<PrivacySettings>): Promise<void> => {
  ...
  const updateDataRetentionSettings = useCallback(
    async (retention: Partial<DataRetentionPreference>): Promise<void> => {
  ...
  const reportPrivacyIncident = useCallback(
    async (incident: PrivacyIncident): Promise<void> => {
      if (!userId) throw new Error("User not authenticated");
  ...
  const acknowledgePrivacyAlert = useCallback(
  ...
  const isConsentGranted = useCallback(
    (consentType: keyof ConsentPreferences): boolean => {
  ...
  const getDataRetentionPeriod = useCallback(
    (dataType: keyof DataRetentionPreference): number => {
  ...
  const contextValue: GDPRContextValue = {
    ...
    consentPreferences:
      state.consentPreferences ||
      (consentQuery.data as ConsentPreferences) ||
      null,
    privacyReport:
      state.privacyReport || (privacyReportQuery.data as PrivacyReport) || null,
    complianceStatus: state.complianceStatus,
    userRights: state.userRights,
    isLoading:
      state.isLoading || consentQuery.isLoading || privacyReportQuery.isLoading,
    isUpdating:
      state.isUpdating ||
      consentUpdateMutation.isPending ||
  ...
  return (
    <GDPRContext.Provider value={contextValue}>{children}</GDPRContext.Provider>
  );
}
