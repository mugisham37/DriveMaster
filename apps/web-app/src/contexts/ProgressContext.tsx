...
export type ProgressAction =
  ...
  | {
      ...
      payload: { topic: string; attempts: AttemptRecord[] };
    }
  ...
  | {
      ...
      payload: { topic: string; mastery: SkillMastery };
    }
  ...
  | {
      type: "TOPIC_COMPARISON_UPDATE";
      payload: { comparisons: TopicComparison[] };
    }
...
function progressReducer(
  ...
  switch (action.type) {
    ...
    case "MASTERY_UPDATE_SUCCESS":
      ...
      updatedMasteries.set(
        action.payload.mastery.topic,
...
export function ProgressProvider({ children }: ProgressProviderProps) {
  ...
  const progressUpdateMutation = useMutation({
    mutationFn: async ({
      ...
      return await userServiceClient.updateSkillMastery(
        userId,
        topic,
        attempts,
      );
  ...
  const updateProgress = useCallback(
    async (topic: string, attempts: AttemptRecord[]) => {
      ...
      try {
        ...
        if (currentMastery && attempts.length > 0) {
          ...
          const { ProgressCalculationManager } = await import(
            "@/lib/user-service/progress-calculation"
          );
          const calculator = new ProgressCalculationManager();

          const immediateFeedback = calculator.calculateImmediateFeedback(
            currentMastery,
            attempts,
          );
  ...
  const getStreakHistory = useCallback(
    async (days: number): Promise<StreakPoint[]> => {
      if (!userId) return [];
      ...
      try {
        ...
        for (let i = days - 1; i >= 0; i--) {
          ...
          const daysSinceStart = Math.floor(
            (today.getTime() - streak.streakStartDate.getTime()) /
              (1000 * 60 * 60 * 24),
          );
          const isActive =
            i <= daysSinceStart && daysSinceStart < streak.currentStreak;

          history.push({
            date,
            streak: isActive
              ? Math.max(1, streak.currentStreak - (daysSinceStart - i))
              : 0,
            active: isActive,
          });
        }
  ...
  const checkMilestoneAchievements = useCallback(async (): Promise<
    Milestone[]
  > => {
    if (!userId) return [];
  ...
  const getMilestoneProgress = useCallback(async (): Promise<
    MilestoneProgress[]
  > => {
    ...
    return state.milestones.map((milestone) => {
      ...
      if (progress > 0 && progress < 1 && state.summary) {
        const daysActive = Math.floor(
          (new Date().getTime() -
            new Date(state.summary.generatedAt).getTime()) /
            (1000 * 60 * 60 * 24),
        );
        if (daysActive > 0) {
  ...
  const getProgressTrends = useCallback(
    async (timeRange: TimeRange): Promise<ProgressTrend[]> => {
      if (!userId || !state.summary) return [];

      try {
        // Use the new ProgressAnalyticsManager for comprehensive trend analysis
        const { ProgressTrendAnalyzer } = await import(
          "@/lib/user-service/progress-analytics"
        );

        const trend = ProgressTrendAnalyzer.analyzeProgressTrends(
  ...
  const getTopicComparison = useCallback(
    async (topics: string[]): Promise<TopicComparison[]> => {
      if (!userId || !state.skillMasteries.size) return [];

      try {
        // Use the new TopicComparisonAnalyzer for peer benchmarking
        const { TopicComparisonAnalyzer } = await import(
          "@/lib/user-service/progress-analytics"
        );

        const comparisons =
          await TopicComparisonAnalyzer.generateTopicComparisons(
            topics,
  ...
  const getPeerComparison = useCallback(
    async (countryCode?: string): Promise<PeerComparison> => {
      ...
      try {
        ...
        const { PeerComparisonAnalyzer } = await import(
          "@/lib/user-service/progress-analytics"
        );

        const comparison = await PeerComparisonAnalyzer.generatePeerComparison(
          state.summary,
          countryCode,
        );
  ...
  const getProgressChartData = useCallback(
    async (type: ChartType): Promise<ChartData> => {
      const cacheKey = `${type}-${userId}`;
      const cached = state.chartDataCache.get(cacheKey);

      if (
        cached &&
        new Date().getTime() - cached.generatedAt.getTime() < 5 * 60 * 1000
      ) {
        return cached;
      }
      ...
      try {
        ...
        const { ChartDataGenerator } = await import(
          "@/lib/user-service/progress-analytics"
        );
        ...
        const chartData = ChartDataGenerator.generateProgressChartData(
          type,
          progressData,
        );

        dispatch({
          type: "CHART_DATA_CACHE",
          payload: { key: cacheKey, data: chartData },
        });
        return chartData;
      } catch (error) {
  ...
  const getMasteryHeatmapData = useCallback(async (): Promise<HeatmapData> => {
    if (
      ...
      new Date().getTime() - state.heatmapDataCache.generatedAt.getTime() <
        10 * 60 * 1000
    ) {
    ...
    try {
      ...
      const { HeatmapDataGenerator } = await import(
        "@/lib/user-service/progress-analytics"
      );
    ...
    try {
      const progressChannel = new ProgressChannel(userId, {
        onProgressUpdate: (event) => {
          ...
          const cacheManager = getUserServiceCacheManager();
          cacheManager
            .invalidateProgress(userId, { topic: event.topic })
            .catch(console.warn);
        },
      ...
      progressChannel
        ...
        .catch((error) => {
          console.warn(
            "Failed to subscribe to real-time progress updates:",
            error,
          );
  ...
  const generateProgressPredictions = useCallback(async () => {
    ...
    try {
      const { ProgressCalculationManager, LearningPatternAnalyzer } =
        await import("@/lib/user-service/progress-calculation");
  ...
  const prefetchProgressData = useCallback(
    async (trigger: string) => {
      ...
      try {
        ...
        const calculator = new ProgressCalculationManager();
        const strategies = calculator.generatePrefetchStrategies(
          learningPattern,
          state.summary,
        );

        // Execute high-priority prefetch strategies
        const highPriorityStrategies = strategies.filter(
          (s) => s.priority === "high",
        );
            ...
            if (strategy.topics.length > 0) {
              for (const topic of strategy.topics) {
                ...
                queryClient.prefetchQuery({
                  queryKey: queryKeys.skillMastery(userId, topic),
                  queryFn: () =>
                    userServiceClient.getSkillMastery(userId, topic),
  ...
  const getSkillMastery = useCallback(
    (topic: string): SkillMastery | null => {
  ...
  const isTopicMastered = useCallback(
    (topic: string, threshold: number = 0.8): boolean => {
      const mastery = state.skillMasteries.get(topic);
  ...
  const contextValue: ProgressContextValue = {
    ...
    // Computed properties
    summary:
      state.summary || (progressSummaryQuery.data as ProgressSummary) || null,
    skillMasteries: state.skillMasteries,
    learningStreak:
      state.learningStreak ||
      (learningStreakQuery.data as LearningStreak) ||
      null,
