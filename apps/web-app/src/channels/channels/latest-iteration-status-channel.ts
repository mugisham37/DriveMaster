...
export type LatestIterationStatusEventHandler = (response: LatestIterationStatusResponse) => void
...
export class LatestIterationStatusChannel {
  ...
  private async subscribe(): Promise<void> {
    ...
    try {
      ...
      console.log(`Subscribed to latest iteration status channel: ${this.uuid}`)

    } catch (error) {
      console.error('Failed to subscribe to latest iteration status channel:', error)
      throw error
    }
  }
  ...
  disconnect(): void {
    ...
    console.log(`Disconnected from latest iteration status channel: ${this.uuid}`)
  }
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on('connected', () => {
      console.log(`Latest iteration status channel connection established: ${this.uuid}`)
    })

    this.connection.on('disconnected', () => {
      console.log(`Latest iteration status channel connection lost: ${this.uuid}`)
    })

    this.connection.on('error', (event) => {
      console.error(`Latest iteration status channel error: ${this.uuid}`, event.error)
    })
  }
  ...
  private handleMessage(data: Record<string, unknown>): void {
    try {
      ...
      if (data.type === 'confirm_subscription') {
        console.log(`Latest iteration status channel subscription confirmed: ${this.uuid}`)
        return
      }

      if (data.type === 'reject_subscription') {
        console.error(`Latest iteration status channel subscription rejected: ${this.uuid}`)
        return
      }
  ...
  private isValidStatusResponse(data: unknown): data is LatestIterationStatusResponse {
    return (
      data !== null &&
      data !== undefined &&
      typeof data === 'object' &&
      ...
      ['queued', 'testing', 'passed', 'failed', 'errored', 'cancelled'].includes((data as Record<string, unknown>).status as string)
    )
  }
}