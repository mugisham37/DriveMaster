...
export type AIHelpRecordsEventHandler = (response: AIHelpRecordsChannelResponse) => void
...
export class AIHelpRecordsChannel {
  ...
  constructor(
    submissionUuid: string,
    onReceive: AIHelpRecordsEventHandler
  ) {
  ...
  private async subscribe(): Promise<void> {
    ...
    try {
      ...
      console.log(`Subscribed to AI help records channel: ${this.submissionUuid}`)
  ...
  disconnect(): void {
    ...
    console.log(`Disconnected from AI help records channel: ${this.submissionUuid}`)
  }
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on('connected', () => {
      console.log(`AI help records channel connection established: ${this.submissionUuid}`)
    })
  ...
  private handleMessage(data: Record<string, unknown>): void {
    try {
      ...
      if (data.type === 'confirm_subscription') {
        console.log(`AI help records channel subscription confirmed: ${this.submissionUuid}`)
        return
      }

      if (data.type === 'reject_subscription') {
        console.error(`AI help records channel subscription rejected: ${this.submissionUuid}`)
        return
      }
