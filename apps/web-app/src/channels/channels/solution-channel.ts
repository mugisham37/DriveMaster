...
export class SolutionChannel {
  ...
  constructor(
    solution: Solution,
    onReceive: SolutionEventHandler
  ) {
    this.solution = solution
    this.onReceive = onReceive
    this.subscriberId = `solution_${solution.uuid}_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on('connected', () => {
      console.log(`Solution channel connection established: ${this.solution.uuid}`)
    })
    ...
    this.connection.on('error', (event: { error?: Error }) => {
      console.error(`Solution channel error: ${this.solution.uuid}`, event.error)
    })
  }
  ...
  private handleMessage(data: Record<string, unknown>): void {
    try {
      ...
      if (data.type === 'confirm_subscription') {
        console.log(`Solution channel subscription confirmed: ${this.solution.uuid}`)
        return
      }

      if (data.type === 'reject_subscription') {
        console.error(`Solution channel subscription rejected: ${this.solution.uuid}`)
        return
      }
  ...
  private isValidSolutionResponse(data: unknown): data is SolutionChannelResponse {
