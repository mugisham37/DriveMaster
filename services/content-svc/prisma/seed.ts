...
const connectionString =
  process.env.DATABASE_URL ?? 'postgresql://localhost:5432/drivemaster_content'
...
const ITEMS_DATA = [
  // Regulatory Signs Items
  {
    ...
    explanation:
      'A stop sign requires you to come to a complete stop, yield to traffic and pedestrians, then proceed when safe.',
    ...
    correctAnswer: {
      answer: 'b',
      explanation: 'Stop signs require a complete stop regardless of traffic conditions',
    },
    points: 1,
    ...
    feedback: {
      ...
      incorrect:
        'Remember that stop signs always require a complete stop, even when no other traffic is visible.',
    },
  ...
  },
  {
    ...
    correctAnswer: {
      answer: 'b',
      explanation: 'Speed limit signs show the maximum legal speed allowed',
    },
    points: 1,
    ...
    feedback: {
      ...
      incorrect:
        'Speed limit signs indicate the maximum speed allowed, not a required or suggested speed.',
    },
  ...
  // Warning Signs Items
  {
    ...
    correctAnswer: {
      answer: 'b',
      explanation: 'Reduce speed before entering curves to maintain control',
    },
    points: 1,
    ...
    feedback: {
      ...
      incorrect:
        'Warning signs alert you to hazards ahead. Reduce speed before curves to maintain vehicle control.',
    },
  ...
  // Right of Way Items
  {
    ...
    explanation:
      'At four-way stops, the vehicle on the right has the right of way when vehicles arrive simultaneously.',
    ...
    correctAnswer: {
      answer: 'b',
      explanation: 'Vehicle to the right has right of way in simultaneous arrivals',
    },
    points: 2,
    hints: [
      'Remember the "right has right of way" rule',
      'This applies when arrival times are simultaneous',
    ],
    feedback: {
      correct: 'Excellent! The vehicle to your right has the right of way at four-way stops.',
      incorrect:
        'At four-way stops, when vehicles arrive simultaneously, the vehicle to your right has the right of way.',
    },
  ...
  // Defensive Driving Items
  {
    ...
    explanation:
      'The 3-second rule provides adequate stopping distance in normal driving conditions.',
    ...
    correctAnswer: {
      answer: 'c',
      explanation: 'The 3-second rule allows adequate reaction and stopping time',
    },
    points: 1,
    hints: [
      'Count "one thousand one" to measure seconds',
      'More distance is needed in poor conditions',
    ],
    feedback: {
      correct: 'Perfect! The 3-second rule provides safe following distance in normal conditions.',
      incorrect:
        'The 3-second rule is recommended for normal conditions, providing adequate reaction time.',
    },
  ...
  // Scenario-based items
  {
    ...
    explanation:
      'In a skid, ease off the accelerator and steer in the direction you want to go, avoiding sudden movements.',
    conceptKey: 'defensive_driving',
    type: 'SCENARIO',
    difficulty: 0.7,
    ...
    correctAnswer: {
      answer: 'c',
      explanation: 'Gentle steering and easing off acceleration helps regain traction',
    },
    points: 3,
    // cSpell:ignore overcorrect
    hints: ['Avoid sudden movements', 'Steer where you want to go', "Don't overcorrect"],
    feedback: {
      correct:
        'Excellent judgment! Smooth steering and reducing acceleration helps regain control.',
      incorrect:
        'In a skid, avoid sudden movements. Ease off the accelerator and steer smoothly in your intended direction.',
    },
...
async function seedContent(): Promise<void> {
  ...
  try {
    ...
    await db.delete(conceptPrerequisites)
    ...
    for (const categoryData of CATEGORIES_DATA) {
      ...
      if (category) {
        categoriesMap.set(categoryData.key, category.id)
    ...
    for (const conceptData of CONCEPTS_DATA) {
      const categoryId = categoriesMap.get(conceptData.categoryKey)
      ...
      if (concept) {
        conceptsMap.set(conceptData.key, concept.id)
    ...
    for (const dep of dependencies) {
      const conceptId = conceptsMap.get(dep.concept)
      const prerequisiteId = conceptsMap.get(dep.prerequisite)

      if (
        conceptId !== undefined &&
        conceptId.length > 0 &&
        prerequisiteId !== undefined &&
        prerequisiteId.length > 0
      ) {
    ...
    for (const itemData of ITEMS_DATA) {
      const conceptId = conceptsMap.get(itemData.conceptKey)
      if (conceptId === undefined || conceptId.length === 0) {
        console.error(`❌ Concept not found: ${itemData.conceptKey}`)
        continue
      }

      // Generate slug
      const baseSlug = itemData.title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
      ...
      const [item] = await db
        ...
        .values({
          ...
          difficultyLevel: itemData.difficultyLevel as
            | 'BEGINNER'
            | 'INTERMEDIATE'
            | 'ADVANCED'
            | 'EXPERT',
          estimatedTime: itemData.estimatedTime,
          options: itemData.options as ItemOptions,
    ...
    for (const [, conceptId] of conceptsMap) {
      const itemCountResult = await db
        .select({ count: sql`count(*)` })
        .from(items)
        .where(sql`${items.conceptId} = ${conceptId} AND ${items.isActive} = true`)

      const itemCount = Number(itemCountResult[0]?.count ?? 0)

      const avgDifficultyResult = await db
        .select({ avg: sql`avg(${items.difficulty})` })
        .from(items)
        .where(sql`${items.conceptId} = ${conceptId} AND ${items.isActive} = true`)

      const avgDifficulty = Number(avgDifficultyResult[0]?.avg ?? 0.5)
      ...
      await db
        ...
        .where(sql`${concepts.id} = ${conceptId}`)
    }

    console.log('🎉 Content seeding completed successfully!')
    console.log(`📈 Summary:`)
    console.log(`   - Categories: ${categoriesMap.size}`)
    console.log(`   - Concepts: ${conceptsMap.size}`)
