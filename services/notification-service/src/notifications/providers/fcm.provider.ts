...
export class FCMProvider implements INotificationProvider {
        ...
        try {
            ...
            if (notification.deviceTokens.length === 1) {
                ...
                const failedTokens = response.responses
                    .map((resp, idx) => resp.success ? null : notification.deviceTokens[idx])
    ...
    private buildMessage(notification: NotificationPayload): admin.messaging.Message {
    ...
    private buildAndroidConfig(notification: NotificationPayload): admin.messaging.AndroidConfig {
        const priority = this.mapPriorityToAndroid(notification.priority);
    ...
    private buildApnsConfig(notification: NotificationPayload): admin.messaging.ApnsConfig {
        return {
            headers: {
    ...
    private buildWebpushConfig(notification: NotificationPayload): admin.messaging.WebpushConfig {
        return {
            notification: {
                title: notification.title,
                body: notification.body,
