...
// Note: In actual implementation, PrismaClient would be imported and instantiated
// const prisma = new PrismaClient()
const prisma: PrismaClientType = {} as PrismaClientType
...
async function main() {
  ...
  for (let i = 0; i < 100; i++) {
    const userId = users[Math.floor(Math.random() * users.length)] ?? 'default-user'
  ...
  for (const userId of users) {
    ...
    await prisma.userBehaviorProfile.create({
      data: {
        ...
        successProbability:
          segment === 'high_achiever' ? Math.random() * 0.3 + 0.7 : Math.random() * 0.6 + 0.2,
  ...
  for (let i = 0; i < 20; i++) {
    const alertType = alertTypes[Math.floor(Math.random() * alertTypes.length)] ?? 'system_anomaly'
    const severity = severities[Math.floor(Math.random() * severities.length)]
    const status = statuses[Math.floor(Math.random() * statuses.length)]
    const entityType = Math.random() > 0.5 ? 'user' : 'concept'
    const entityId =
      entityType === 'user'
        ? users[Math.floor(Math.random() * users.length)] ?? 'default-user'
        : concepts[Math.floor(Math.random() * concepts.length)] ?? 'default-concept'
    ...
    await prisma.alert.create({
      data: {
        ...
        resolvedAt:
          status === 'RESOLVED' ? new Date(Date.now() - Math.random() * 24 * 60 * 60 * 1000) : null,
  ...
  for (let i = 0; i < 50; i++) {
    const modelName =
      predictionModels[Math.floor(Math.random() * predictionModels.length)]?.modelName ?? 'default-model'
    ...
    },
    {
      ...
      layout: {
        widgets: [
          ...
          {
            type: 'metric_card',
            metric: 'predictions_today',
            position: { x: 2, y: 0, w: 2, h: 2 },
          },
          {
            type: 'chart',
            metric: 'dropout_risk_distribution',
            position: { x: 0, y: 2, w: 4, h: 3 },
          },
          {
            type: 'chart',
            metric: 'model_performance_trends',
    ...
    for (const step of funnel.steps) {
      await prisma.funnelStep.create({
        data: {
          ...
          avgTimeToNext:
            step.order < funnel.steps.length ? Math.floor(Math.random() * 86400) + 3600 : null, // 1-24 hours
          dropoffReasons:
            step.order > 1
              ? {
  ...
  console.log(
    `- ${metricNames.length * timeWindows.length * users.length * concepts.length * 10} metric aggregations`,
  )
