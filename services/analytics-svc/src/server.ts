...
let eventProcessor: AnalyticsEventProcessor | null = null
if (env.KAFKA_BROKERS && env.REDIS_URL) {
  eventProcessor = new AnalyticsEventProcessor(
    {
...
class AnalyticsProcessor {
  ...
  static async calculateEngagementRisk(userId: string, events: any[]): Promise<number> {
    ...
    const avgEngagement =
      engagementEvents.reduce((sum, e) => sum + e.engagementScore, 0) / engagementEvents.length
...
async function startEventProcessor() {
  if (eventProcessor) {
    try {
      ...
      app.log.info('Analytics Event Processor started successfully')
    } catch (error) {
      app.log.error({ error }, 'Failed to start Analytics Event Processor')
    }
  }
}

// Event replay endpoint for ML training
app.post(
  '/events/replay',
  {
    preHandler: [requireAdmin],
  ...
  async (request: any, reply) => {
    try {
      const { fromTimestamp, toTimestamp } = request.body

      if (!eventProcessor) {
        return reply.code(503).send({ error: 'Event processor not available' })
      }

      const from = new Date(fromTimestamp)
      const to = toTimestamp ? new Date(toTimestamp) : undefined

      await eventProcessor.replayEvents(from, to)
      ...
      app.log.error(error, 'Event replay error')
      reply.code(500).send({ error: 'Internal server error' })
    }
  },
)
...
app.get(
  ...
  async (request, reply) => {
    try {
      const processingStats = await prisma.learningEventStream.groupBy({
        by: ['processed'],
        _count: {
          processed: true,
        },
      ...
      const totalEvents = processingStats.reduce((sum, stat) => sum + stat._count.processed, 0)
      const processedEvents = processingStats.find((stat) => stat.processed)?._count.processed || 0
      const pendingEvents = processingStats.find((stat) => !stat.processed)?._count.processed || 0
      ...
      reply.send({
        ...
        eventProcessorStatus: eventProcessor ? 'running' : 'stopped',
      })
    } catch (error) {
      app.log.error(error, 'Event status error')
      reply.code(500).send({ error: 'Internal server error' })
    }
  },
)
...
app
  .listen({ port: env.PORT || 3004, host: '0.0.0.0' })
  .then(async () => {
    app.log.info(`analytics-svc listening on ${env.PORT || 3004}`)
    // Start event processor after server is ready
