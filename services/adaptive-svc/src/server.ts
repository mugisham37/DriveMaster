  ...
  try {
    ...
    const context: BKTUpdateContext = {
      ...
      timeSinceLastInteraction: knowledgeState ? 
        Date.now() - knowledgeState.lastInteraction.getTime() : undefined
    }
  ...
  try {
    ...
    const features: ContextualFeatures = {
      ...
      previousPerformance: knowledgeState ? 
        knowledgeState.correctAnswers / Math.max(knowledgeState.totalInteractions, 1) : 0.5,
...
app.get('/knowledge-state/:userId/:conceptKey', {
  preHandler: [authenticate]
  ...
  try {
    ...
    reply.send({
      ...
      statistics: {
        ...
        accuracyRate: knowledgeState.correctAnswers / Math.max(knowledgeState.totalInteractions, 1),
...
app.get('/due-reviews/:userId', {
  ...
  try {
    ...
    const reviews = dueReviews.map(review => {
      const overdueDays = Math.max(0, (Date.now() - review.nextReview.getTime()) / (24 * 60 * 60 * 1000))
