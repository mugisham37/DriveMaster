...
export class MultiArmedBandit {
  ...
  static calculateContextualReward(
    ...
    const progressionReward =
      this.calculateProgressionReward(arm.difficulty, userMastery, previousPerformance) *
      difficultyWeight
  ...
  static calculateTimeAdjustment(timeOfDay: string): number {
    ...
    if (hourStr === undefined || hourStr === null || hourStr.length === 0) return 0.8

    const hour = parseInt(hourStr, 10)
    if (isNaN(hour)) return 0.8
   ...
   */
  static calculateRegret(optimalReward: number, actualReward: number): number {
   ...
   */
  static getExplorationRatio(stats: Map<string, BanditArmStats>): {
    exploration: number
    exploitation: number
  } {
   ...
   */
  static detectConceptDrift(stats: Map<string, BanditArmStats>, windowSize: number = 100): boolean {
   ...
   */
  static resetBanditStats(stats: Map<string, BanditArmStats>, decayFactor: number = 0.5): void {
    for (const [_armId, armStats] of stats) {
      const decayedStats: BanditArmStats = {
        ...
        lastPulled: armStats.lastPulled ?? new Date(),
      }

      stats.set(_armId, decayedStats)
    }
  }
}

// Legacy function for backward compatibility
export function selectNextItem(candidates: Array<{ id: string; score: number }>): {
  id: string
  score: number
} {
