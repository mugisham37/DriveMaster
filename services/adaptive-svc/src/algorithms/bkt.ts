...
export class BayesianKnowledgeTracing {
  ...
  static updateKnowledgeState(
    ...
    const { pL0, pT, pG, pS, learningVelocity, confidenceWeight, responseTimeWeight } = params
    ...
    if (responseTime && responseTimeWeight > 0) {
    ...
    if (confidence && confidenceWeight > 0) {
  ...
  static estimateInitialParams(
    ...
    if (userProfile?.cognitiveProfile) {
      const profile = userProfile.cognitiveProfile
      ...
      if (profile.confidenceCorrelation === 'high') {
        defaults.confidenceWeight = 0.2
      }
      
      if (profile.responseTimeCorrelation === 'high') {
   ...
   */
  static predictCorrectProbability(
    mastery: number,
    pG: number,
    pS: number
  ): number {
