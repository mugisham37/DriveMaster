...
export class ItemResponseTheory {
  ...
  static calculateFisherInformation(
    ...
    return itemParams.reduce((total, params) => 
      total + this.calculateInformation(theta, params), 0
    )
  }
  ...
  static estimateAbility(
    ...
    const information = this.calculateTestInformation(theta, responses.map(r => r.params))
  ...
  static calculateLogLikelihoodDerivatives(
    ...
    for (const response of responses) {
      ...
      const first = a * p_star * (correct / probability - (1 - correct) / (1 - probability))
      firstDerivative += first
      
      // Second derivative
      const second = -a * a * p_star * exponent / (denominator * denominator) * 
                    (correct / (probability * probability) + (1 - correct) / ((1 - probability) * (1 - probability)))
  ...
  static calibrateItem(
    ...
    if (responses.length < 10) {
      ...
      return this.getDefaultItemParams(itemData.correctResponses / Math.max(itemData.totalResponses, 1))
    }
  ...
  static estimateDiscrimination(
    ...
    const abilities = validResponses.map(r => r.theta!)
  ...
  static calculateExpectedScore(theta: number, itemParams: IRTItemParams[]): number {
    return itemParams.reduce((score, params) => 
      score + this.calculateProbability(theta, params), 0
    )
  }
