...
export class SpacedRepetition {
  ...
  static updateSpacedRepetition(
    ...
    if (quality >= this.QUALITY_THRESHOLD) {
      ...
      newEaseFactor = Math.max(this.MIN_EASE_FACTOR, easeFactor + adjustment)
    } else {
    ...
    // Cap the ease factor
    newEaseFactor = Math.min(this.MAX_EASE_FACTOR, newEaseFactor)
    ...
    const forgettingAdjustment = 1 - (forgettingCurve * personalizedDecay)
    ...
    const confidence = this.calculateScheduleConfidence(
      quality,
      newRepetitions,
      newEaseFactor,
      forgettingCurve
    ...
    return { newParams, nextReview, confidence }
  }
  ...
  static calculateScheduleConfidence(
    ...
    const easeConfidence = (easeFactor - this.MIN_EASE_FACTOR) / (this.MAX_EASE_FACTOR - this.MIN_EASE_FACTOR)
  ...
  static findOptimalReviewTime(
    ...
    const baseReviewTime = new Date(context.currentTime.getTime() + params.interval * 24 * 60 * 60 * 1000)
    ...
    const inPreferredTime = preferredTimes.some(time => {
      ...
      return Math.abs(reviewHour - hour) <= 2
    })
    
    // Check if falls in avoid time
    const inAvoidTime = avoidTimes.some(time => {
      const [hour] = time.split(':').map(Number)
      return Math.abs(reviewHour - hour) <= 1
    })
    ...
    return {
      ...
      probability,
      confidence: this.calculateScheduleConfidence(5, params.repetitions, params.easeFactor, params.forgettingCurve),
      burden: params.reviewBurden
    }
  }
    ...
    for (const [dateKey, itemIds] of reviewsByDate) {
      if (itemIds.length <= maxDailyReviews) {
        ...
        for (let i = maxDailyReviews; i < itemIds.length; i++) {
          ...
          balancedSchedule.set(itemIds[i], newDate)
        }
      }
    }
  ...
  static getDueItems(
    ...
    for (const [itemId, schedule] of scheduledReviews) {
      if (schedule.date <= cutoffTime) {
        ...
        const overdueDays = Math.max(0, (currentTime.getTime() - schedule.date.getTime()) / (24 * 60 * 60 * 1000))
...
export function scheduleNext(quality: number, correct: boolean): Date {
